...
namespace
{
    ...
    NetNewUserCallback g_new_user_callback = nullptr;
    const EnetConnectivityServices *g_connectivity_services = nullptr;
    ...
    TbError bf_enet_init(NetDropCallback drop_callback, NetNewUserCallback new_user_callback)
    {
        if (enet_initialize())
            return Lb_FAIL;
        g_drop_callback = drop_callback;
        g_new_user_callback = new_user_callback;
        return Lb_OK;
    }
    ...
    void bf_enet_exit()
    {
        if (g_connectivity_services)
            g_connectivity_services->port_forward_remove_mapping();
        host_destroy();
        g_drop_callback = nullptr;
        g_new_user_callback = nullptr;
        enet_deinitialize();
    }
    ...
    TbError bf_enet_host(const char *session, void *)
    {
        ...
        if (g_connectivity_services) {
            g_connectivity_services->port_forward_add_mapping(address.port);
            external_ipv4_port = g_connectivity_services->holepunch_stun_query(host, NULL, 0);
        }
        return Lb_OK;
    }
    ...
    TbError join_via_holepunch(TbClockMSec join_start_ms)
    {
        ...
        if (!g_connectivity_services) {
            LbNetLog("Join: no connectivity services registered\n");
            return Lb_FAIL;
        }
        ...
        while (LbTimerClock() < punch_learn_deadline) {
            ...
            SDL_Delay(100);
            if (has_ipv6 && !learned_ipv6) learned_ipv6 = g_connectivity_services->holepunch_receive(ipv6_host, &ipv6_address, 1);
            if (has_ipv4 && !learned_ipv4) learned_ipv4 = g_connectivity_services->holepunch_receive(host, &ipv4_address, 1);
        ...
        while (LbTimerClock() < holepunch_deadline) {
            ...
            if (ipv6_peer) g_connectivity_services->holepunch_punch_to(ipv6_host, &ipv6_address);
            if (ipv4_peer) g_connectivity_services->holepunch_punch_to(host, &ipv4_address);
    ...
    TbError bf_enet_join(const char *session, void *)
    {
        ...
        if (strncmp(g_join_lobby_id, "LAN:", 4) == 0) {
            LbNetLog("Join: connecting via LAN\n");
            char lan_peer_address[ENET_MATCHMAKING_IP_MAX];
            snprintf(lan_peer_address, sizeof(lan_peer_address), "%s", g_join_lobby_id + 4);
            g_join_lobby_id[0] = '\0';
    ...
    bool wait_for_incoming_packet(NetUserId source, unsigned timeout)
    {
        ...
        while (true)
        {
            ...
            if (!client_peer) {
                new_user_callback = g_new_user_callback;
            } else {
...
static bool IsLocalPeer(NetUserId id, NetUserId local_player_id) {
...
int enet_matchmaking_host_update(void)
{
    ...
    if (pending_punch_ipv6.port) {
        g_connectivity_services->holepunch_punch_to(host, &pending_punch_ipv6);
    }
    if (pending_punch_ipv4.port) {
        g_connectivity_services->holepunch_punch_to(host, &pending_punch_ipv4);
    }
...
{
    g_connectivity_services = services;
}

void bf_enet_set_join_lobby_id(const char *lobby_id)
{
    snprintf(g_join_lobby_id, sizeof(g_join_lobby_id), "%s", lobby_id ? lobby_id : "");
}
