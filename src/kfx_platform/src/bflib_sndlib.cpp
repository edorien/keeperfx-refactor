...
// See bf_sndlib_set_audio_config() and docs/refactor/stage-02-decouple-bflib.md.
...
extern "C" void bf_sndlib_set_audio_config(const char *language_lwrstr, TbBool no_cd_music) {
	snprintf(g_audio_language_lwrstr, sizeof(g_audio_language_lwrstr), "%s", language_lwrstr);
	g_audio_no_cd_music = no_cd_music;
}
...
extern "C" TbBool InitAudio(unsigned char max_number_of_samples) {
	try {
		...
		g_sources.resize(max_number_of_samples);
