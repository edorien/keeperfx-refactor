...
static SoundSmplTblID gui_snd_button_click = 0;
static SoundSmplTblID gui_snd_tab_click = 0;

void set_gui_click_sound_ids(SoundSmplTblID button_click_id, SoundSmplTblID tab_click_id)
{
    gui_snd_button_click = button_click_id;
    gui_snd_tab_click = tab_click_id;
}

void do_sound_menu_click(void)
{
    play_non_3d_sample_no_overlap(gui_snd_button_click);
}

void do_sound_button_click(struct GuiButton *gbtn)
{
    if (gbtn->gbtype == LbBtnT_RadioBtn)
        play_non_3d_sample(gui_snd_tab_click);
    else
        play_non_3d_sample(gui_snd_button_click);
}
