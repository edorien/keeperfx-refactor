...
// See struct KeeperFxUiConfig and docs/refactor/stage-04-kfx-config.md issue B.
struct KeeperFxUiConfig keeperfx_ui_config = {
    .gui_blink_rate = 1,
    .neutral_flash_rate = 1,
    .creature_status_size = 16,
    .line_box_size = 150,
    .right_click_tag_mode_toggle = false,
    .default_tag_mode = 1,
...
const struct NamedCommand conf_commands[] = {
  ...
  // Values mirror front_input.h's enum ZoomToMouseOptions (not included
  // here -- see struct KeeperFxUiConfig and
  ...
  // Values mirror front_input.h's enum RotateAroundMouseOptions.
  const struct NamedCommand rotate_around_mouse_options[] = {
  {"ROTATION_KEYS", 2}, // RotateAroundMouse_RotationKeys
  {"MOVEMENT_KEYS", 3}, // RotateAroundMouse_MovementKeys
  {NULL,            0},
  };
...
{
    if (g_network_is_active_fn != NULL && g_network_is_active_fn())
    {
        return false;
    }
...
#define COMMAND_TEXT(cmd_num) get_conf_parameter_text(conf_commands,cmd_num)
    while (pos<len)
    {
      ...
      case 42: // ZOOM_TO_MOUSE
          ...
          if (i == 1)
          {
              keeperfx_ui_config.zoom_to_mouse_option = 3; // ZoomToMouse_Always
              break;
          }
          else if (i == 2)
          {
              keeperfx_ui_config.zoom_to_mouse_option = 1; // ZoomToMouse_Never
              break;
          }
          else if (i <= 0)
          {
              ...
              if (i > 0)
              {
                  keeperfx_ui_config.zoom_to_mouse_option = i;
                  break;
              }
          }
          CONFWRNLOG("Couldn't recognize \"%s\" command parameter in %s file.",
      ...
      case 43: // ROTATE_AROUND_MOUSE
          ...
          if (i == 1)
          {
              keeperfx_ui_config.rotate_around_mouse_option = 4; // RotateAroundMouse_Always
              break;
          }
          else if (i == 2)
          {
              keeperfx_ui_config.rotate_around_mouse_option = 1; // RotateAroundMouse_Never
              break;
          }
          else if (i <= 0)
          {
              ...
              if (i > 0)
              {
                keeperfx_ui_config.rotate_around_mouse_option = i;
                break;
              }
          }
          CONFWRNLOG("Couldn't recognize \"%s\" command parameter in %s file.",
