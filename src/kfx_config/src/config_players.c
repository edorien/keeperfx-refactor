...
// Moved from kfx_game's lvl_script_commands.c -- see the doc comment at
// its declaration in config_players.h.
...
static TbBool load_playerstate_config_file(const char *fname, unsigned short flags)
{
    ...
    for (int id = 0; id < PLAYER_STATES_COUNT_MAX; id++)
    {
        ...
        {
            struct PlayerStateConfigStats *plrst_cfg_stat = &kfx_config_state.conf.plyr_conf.plrst_cfg_stats[id];
...
{
    return &kfx_config_state.conf.plyr_conf.plrst_cfg_stats[plyr_state];
}
