...
const struct NamedFieldSet terrain_slab_named_fields_set = {
    ...
    slab_desc,
    TERRAIN_ITEMS_MAX,
    sizeof(kfx_config_state.conf.slab_conf.slab_cfgstats[0]),
    get_slab_base,
};
...
const struct NamedFieldSet terrain_room_named_fields_set = {
    ...
    room_desc,
    TERRAIN_ITEMS_MAX,
    sizeof(kfx_config_state.conf.slab_conf.room_cfgstats[0]),
    get_room_base,
};
...
{
    if (room_kind >= kfx_config_state.conf.slab_conf.room_types_count)
        return &kfx_config_state.conf.slab_conf.room_cfgstats[0];
    return &kfx_config_state.conf.slab_conf.room_cfgstats[room_kind];
}
