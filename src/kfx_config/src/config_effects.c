...
#endif
/******************************************************************************/
// Duplicated from kfx_sim's map_data.h (CONFIG_COORD_PER_STL) rather than
// included, to avoid a kfx_config -> kfx_sim layering violation for a
...
const struct NamedFieldSet effects_effectgenerator_named_fields_set = {
    ...
    sizeof(kfx_config_state.conf.effects_conf.effectgen_cfgstats[0]),
    get_effectgen_base,
};
...
static void load_effects(VALUE *value, unsigned short flags)
{
    ...
    for (int id = 0; id < EFFECTS_TYPES_MAX; id++)
    {
        ...
        {
            struct EffectConfigStats *effcst = &kfx_config_state.conf.effects_conf.effect_cfgstats[id];
    ...
    if (max_effect_id >= 0)
    {
        ...
        for (int id = 0; id <= max_effect_id; id++)
        {
            if (effect_desc[id].name == NULL)
            {
                ...
                effect_desc[id].name = kfx_config_state.conf.effects_conf.effect_cfgstats[id].code_name;
            }
        }
    }
}
...
static void load_effectsgenerators(VALUE *value, unsigned short flags)
{
    ...
    for (int id = 0; id < EFFECTSGEN_TYPES_MAX; id++)
    {
        ...
        if (value_type(section) == VALUE_DICT)
        {
            struct EffectGeneratorConfigStats *effgencst = &kfx_config_state.conf.effects_conf.effectgen_cfgstats[id];
    ...
    if (max_effectgen_id >= 0)
    {
        ...
        for (int id = 0; id <= max_effectgen_id; id++)
        {
            if (effectgen_desc[id].name == NULL)
            {
                ...
                effectgen_desc[id].name = kfx_config_state.conf.effects_conf.effectgen_cfgstats[id].code_name;
            }
        }
    }
}
...
static void load_effectelements(VALUE *value, unsigned short flags)
{
    ...
    for (int id = 0; id < EFFECTSELLEMENTS_TYPES_MAX; id++)
    {
        ...
        if (value_type(section) == VALUE_DICT)
        {
            struct EffectElementConfigStats *effelcst = &kfx_config_state.conf.effects_conf.effectelement_cfgstats[id];
    ...
    if (max_effectelement_id >= 0)
    {
        ...
        for (int id = 0; id <= max_effectelement_id; id++)
        {
            if (effectelem_desc[id].name == NULL)
            {
                ...
                effectelem_desc[id].name = kfx_config_state.conf.effects_conf.effectelement_cfgstats[id].code_name;
            }
        }
    }
}
...
struct EffectGeneratorConfigStats *get_effectgenerator_model_stats(ThingModel tngmodel)
{
    if (tngmodel >= EFFECTSGEN_TYPES_MAX)
        return &kfx_config_state.conf.effects_conf.effectgen_cfgstats[0];
    return &kfx_config_state.conf.effects_conf.effectgen_cfgstats[tngmodel];
}

struct EffectConfigStats *get_effect_model_stats(ThingModel tngmodel)
{
    if (tngmodel >= EFFECTS_TYPES_MAX)
        return &kfx_config_state.conf.effects_conf.effect_cfgstats[0];
