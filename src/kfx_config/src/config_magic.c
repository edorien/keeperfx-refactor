...
const struct NamedFieldSet magic_shot_named_fields_set = {
    ...
    shot_desc,
    MAGIC_ITEMS_MAX,
    sizeof(kfx_config_state.conf.magic_conf.shot_cfgstats[0]),
...
const struct NamedFieldSet magic_powers_named_fields_set = {
    ...
    power_desc,
    MAGIC_ITEMS_MAX,
    sizeof(kfx_config_state.conf.magic_conf.power_cfgstats[0]),
    get_powers_base,
};
...
{
    if ((spell_idx == 0) || (spell_idx >= kfx_config_state.conf.magic_conf.spell_types_count))
    {
        return &kfx_config_state.conf.magic_conf.spell_config[0];
    }
    return &kfx_config_state.conf.magic_conf.spell_config[spell_idx];
}

TbBool spell_config_is_invalid(struct SpellConfig *mgcinfo)
{
    if (mgcinfo <= &kfx_config_state.conf.magic_conf.spell_config[0])
    {
        return true;
    }
    return false;
...
TextStringId get_power_name_strindex(PowerKind pwkind)
{
    if (pwkind >= kfx_config_state.conf.magic_conf.power_types_count)
        return kfx_config_state.conf.magic_conf.power_cfgstats[0].name_stridx;
    return kfx_config_state.conf.magic_conf.power_cfgstats[pwkind].name_stridx;
}

TextStringId get_power_description_strindex(PowerKind pwkind)
{
  if (pwkind >= kfx_config_state.conf.magic_conf.power_types_count)
    return kfx_config_state.conf.magic_conf.power_cfgstats[0].tooltip_stridx;
...
{
    if (spmodel >= kfx_config_state.conf.magic_conf.spell_types_count)
        return &kfx_config_state.conf.magic_conf.spell_cfgstats[0];
    return &kfx_config_state.conf.magic_conf.spell_cfgstats[spmodel];
}

struct ShotConfigStats *get_shot_model_stats(ThingModel tngmodel)
{
    if (tngmodel >= kfx_config_state.conf.magic_conf.shot_types_count)
        return &kfx_config_state.conf.magic_conf.shot_cfgstats[0];
    return &kfx_config_state.conf.magic_conf.shot_cfgstats[tngmodel];
}

struct PowerConfigStats *get_power_model_stats(PowerKind pwkind)
{
    if (pwkind >= kfx_config_state.conf.magic_conf.power_types_count)
        return &kfx_config_state.conf.magic_conf.power_cfgstats[0];
    return &kfx_config_state.conf.magic_conf.power_cfgstats[pwkind];
}

TbBool power_model_stats_invalid(const struct PowerConfigStats *powerst)
{
  if (powerst <= &kfx_config_state.conf.magic_conf.power_cfgstats[0])
    return true;
  return false;
}
...
{
    if (spckind >= kfx_config_state.conf.magic_conf.special_types_count)
        return &kfx_config_state.conf.magic_conf.special_cfgstats[0];
...
TbBool parse_magic_special_blocks(char *buf, long len, const char *config_textname, unsigned short flags)
{
  ...
  if ((flags & CnfLd_AcceptPartial) == 0) {
      for (int i = 0; i < MAGIC_ITEMS_MAX; i++) {
          ...
          kfx_config_state.conf.object_conf.object_to_special_artifact[i] = 0;
      }
  }
...
static TbBool load_magic_config_file(const char *fname, unsigned short flags)
{
   ...
   if (!flag_is_set(flags, CnfLd_AcceptPartial)) {
    ...
    kfx_config_state.conf.magic_conf.spell_types_count = MAGIC_ITEMS_MAX;
    kfx_config_state.conf.magic_conf.special_types_count = MAGIC_ITEMS_MAX;
