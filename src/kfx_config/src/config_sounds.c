...
TbBool cache_common_sound_ids(void)
{
    ...
    // bflib_guibtns.c can't read config_sounds.h's globals directly (see
    // docs/refactor/stage-02-decouple-bflib.md), so push the two it needs
    // down explicitly.
    set_gui_click_sound_ids(snd_button_click, snd_tab_click);
