...
void set_room_playing_ambient_sound(struct Coord3d *pos, long sample_idx)
{
    ...
    struct Thing* thing = thing_get(kfx_game_state.ambient_sound_thing_idx);
    if ( thing_is_invalid(thing) )
    {
...
void update_player_sounds(void)
{
    ...
    if (get_gameturn() != 0)
    {
        ...
        if (kfx_config_state.conf.rules[0].gameplay.easter_egg_speech_interval != 0 && (get_gameturn() % kfx_config_state.conf.rules[0].gameplay.easter_egg_speech_interval) == 0)
        {
            // The chance for the easter egg speech to trigger. Original DK value was 1/2000
            if (kfx_config_state.conf.rules[0].gameplay.easter_egg_speech_chance != 0 && SOUND_RANDOM(kfx_config_state.conf.rules[0].gameplay.easter_egg_speech_chance) == 0)
            {
        ...
        else
        {
            ...
            if ( atmos_sounds_enabled() )
            {
                ...
                k = SOUND_RANDOM(kfx_config_state.atmos_sound_frequency);
                if (k == 1)
                {
...
TbBool init_sound(void)
{
    ...
    InitAudio(snd_settng->max_number_of_samples);
...
TbBool ambient_sound_prepare(void)
{
    ...
    struct Thing* thing = create_ambient_sound(&pos, 1, kfx_config_state.neutral_player_num);
    if (thing_is_invalid(thing))
    {
    ...
    kfx_game_state.ambient_sound_thing_idx = thing->index;
    return true;
}
