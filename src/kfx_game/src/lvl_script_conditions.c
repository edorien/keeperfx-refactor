...
long get_condition_value(PlayerNumber plyr_idx, unsigned char valtype, short validx)
{
    ...
    case SVar_AVAILABLE_TRAP: // IF_AVAILABLE(TRAP)
        return count_player_available_traps_of_model(plyr_idx, (validx % kfx_config_state.conf.trapdoor_conf.trap_types_count));
    case SVar_AVAILABLE_DOOR: // IF_AVAILABLE(DOOR)
        return count_player_available_doors_of_model(plyr_idx, (validx % kfx_config_state.conf.trapdoor_conf.door_types_count));
    case SVar_AVAILABLE_ROOM: // IF_AVAILABLE(ROOM)
        dungeon = get_dungeon(plyr_idx);
        return (dungeon->room_buildable[validx%kfx_config_state.conf.slab_conf.room_types_count] & 1);
    case SVar_AVAILABLE_CREATURE: // IF_AVAILABLE(CREATURE)
        return count_player_available_creatures_of_model(plyr_idx, (validx % kfx_config_state.conf.crtr_conf.model_count));
    ...
    case SVar_CONTROLS_CREATURE: // IF_CONTROLS(CREATURE)
        dungeon = get_dungeon(plyr_idx);
        return dungeon->owned_creatures_of_model[validx%kfx_config_state.conf.crtr_conf.model_count]
    ...
    case SVar_BONUS_TIME:
        return (kfx_game_state.bonus_time - get_gameturn());
...
void command_add_condition(long plr_range_id, long opertr_id, long varib_type, long varib_id, long value)
{
    ...
    script_current_condition = kfx_game_state.script.conditions_num;
    kfx_game_state.script.conditions_num++;
}
