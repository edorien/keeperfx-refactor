...
void initialise_devastate_dungeon_from_heart(PlayerNumber plyr_idx)
{
    ...
    if (dungeon->devastation_turn == 0)
    {
        ...
        if (thing_exists(heartng)) {
            ...
            dungeon->devastation_turn = 1;
            dungeon->devastation_centr_x = kfx_sim_state.map_subtiles_x / 2;
            dungeon->devastation_centr_y = kfx_sim_state.map_subtiles_y / 2;
        }
    }
}
...
void process_dungeon_destroy(struct Thing* heartng)
{
    ...
    case 3:
        ...
        if ((dungeon->num_things_in_hand > 0) && ((kfx_config_state.conf.rules[plyr_idx].gameplay.classic_bugs_flags & ClscBug_NoHandPurgeOnDefeat) == 0))
        {
            if (no_backup)
    ...
    { // If there is another heart owned by this player, set it to "working" heart
        ...
        {
            if (kfx_sim_state.heart_lost_display_message)
            {
                if (is_my_player_number(dungeon->owner))
                {
                    const char* objective = (kfx_game_state.heart_lost_quick_message) ? kfx_sim_state.quick_messages[kfx_game_state.heart_lost_message_id] : get_string(kfx_game_state.heart_lost_message_id);
                    process_objective(objective,dungeon->owner, kfx_game_state.heart_lost_message_target, 0, 0);
                }
            }
