...
long get_parchment_map_area_rect(struct TbRect *map_area)
{
    ...
    map_area->right = map_area->left + block_size*kfx_sim_state.map_tiles_x;
    map_area->bottom = map_area->top + block_size*kfx_sim_state.map_tiles_y;
    return block_size;
}
    ...
    *map_y = 0;
      ...
      && (screen_y >= map_area.top) && (screen_y < map_area.bottom))
    {
        ...
        *map_y = COORD_PER_STL * STL_PER_SLB * (screen_y-map_area.top)  / block_size + COORD_PER_STL/2;
        return ((*map_x >= 0) && (*map_x < (kfx_sim_state.map_subtiles_x+1)<<8) && (*map_y >= 0) && (*map_y < (kfx_sim_state.map_subtiles_y+1)<<8));
    }
    return false;
}
    ...
    while (i != 0)
    {
        ...
        if (!thing_is_picked_up(thing))
        {
            ...
            if (thing_revealed(thing, plyr_idx))
            {
                ...
                {
                    col1 = player_room_colours[(((get_gameturn() + kfx_config_state.neutral_flash_rate) % (4 * kfx_config_state.neutral_flash_rate)) / kfx_config_state.neutral_flash_rate)];
                } else
