...
void refresh_active_button_sprites_for_player(PlayerNumber plyr_idx)
{
    for (int btn_idx = 0; btn_idx < ACTIVE_BUTTONS_COUNT; btn_idx++) {
        ...
        if (gbinit != NULL && gbinit->sprite_idx != 0) {
            gbtn->sprite_idx = get_player_colored_icon_idx(gbinit->sprite_idx, plyr_idx);
        }
    }
}
...
void turn_on_menu(MenuID mnu_idx)
{
    ...
    if (create_menu(gmnu) >= 0)
    {
      if (gmnu->is_active_panel)
        kfx_sim_state.active_panel_mnu_idx = mnu_idx;
    }
}
...
void reset_gui_based_on_player_mode(void)
{
    ...
    {
        turn_on_menu(GMnu_MAIN);
        if (kfx_sim_state.active_panel_mnu_idx > 0)
        {
            initialise_tab_tags(kfx_sim_state.active_panel_mnu_idx);
            ...
            {
                turn_on_menu(kfx_sim_state.active_panel_mnu_idx);
            }
