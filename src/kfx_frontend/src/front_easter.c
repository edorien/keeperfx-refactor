...
void input_eastegg(void)
{
    ...
    if (kfx_sim_state.easter_eggs_enabled == true)
    {
      ...
      state = input_eastegg_keycodes(&kfx_frontend_state.eastegg02_cntr,allow,&eastegg_jlw_codes);
...
static void draw_bouncing_eastegg_message(long idx, const char *text, long width, long height, int ee_units_per_px)
{
  ...
  px[idx] += vx[idx] * kfx_render_state.delta_time;
  if (px[idx] < 0)
  {
    px[idx] = 0;
    vx[idx] = -vx[idx];
  }
  py[idx] += vy[idx] * kfx_render_state.delta_time;
  if (py[idx] < 0)
  {
    py[idx] = 0;
    vy[idx] = -vy[idx];
...
void draw_eastegg(void)
{
  ...
  if (kfx_frontend_state.eastegg02_cntr >= eastegg_jlw_codes.length)
  {
