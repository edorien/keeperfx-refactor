...
// game_key_settings[] moved to kfx_config's config_settings.c -- it was
// declared as kfx_config's own extern in config_settings.h (stage 13.3)
// but the data itself stayed here; config_settings.c is the lowest-ranked
// real consumer, so the data moved down to match. See docs/refactor/todo/
...
{
  if (kfx_net_state.frame_skip > 512)
    kfx_net_state.frame_skip = 512;
...
static long get_small_map_inputs(long x, long y, long zoom)
{
  ...
  old_my = curr_my;
  if (grabbed_small_map)
    kfx_sim_state.small_map_state = 2;
  SYNCDBG(8,"Finished");
  return result;
}
...
static short get_global_inputs(void)
{
  ...
  && (network_is_active() ||
     ((kfx_game_state.flags_gui & GGUI_SoloChatEnabled) != 0)))
  {
      if (is_key_pressed(KC_RETURN,KMod_NONE))
      {
  ...
  if ((player->instance_num != PI_MapFadeTo) &&
      ...
      (!game_is_busy_doing_gui_string_input()))
    {
        ...
        if (is_game_key_pressed(Gkey_PauseMenu, true, false))
        {
            ...
            if ((kfx_sim_state.operation_flags & GOF_Paused)) {
                toggle_pause = true;
                clear_key_pressed(KC_ESCAPE);
            }
      ...
      else if( flag_is_set(kfx_sim_state.operation_flags, GOF_Paused) && flag_is_set(start_params.debug_flags, DFlg_FrameStep) )
      {
        if( is_key_pressed(KC_PERIOD, SDL_KMOD_NONE) )
        {
            kfx_game_state.frame_step = true;
            set_packet_pause_toggle();
...
static TbBool get_dungeon_control_pausable_action_inputs(void)
{
    ...
    if (is_game_key_pressed(Gkey_SwitchToMap, true, false))
    {
      ...
      if ((player->view_mode != PVM_ParchFadeOut) && (kfx_sim_state.small_map_state != 2))
      {
          turn_off_all_window_menus();
...
static void set_packet_action_for_thing_under_hand(struct Packet* pckt)
{
    ...
    struct PlayerInfo* player = get_my_player();
    if ((player->view_type != PVT_DungeonTop) || ((pckt->control_flags & PCtr_Gui) != 0) || (local_thing_under_hand <= 0) || (pckt->action != PckA_None) || (get_gameturn() - hand_pick_pending_turn <= kfx_net_state.input_lag_turns)) {
        return;
    }
    ...
    for (GameTurnDelta i = 1; i <= kfx_net_state.input_lag_turns; i += 1) {
...
static TbBool get_player_coords_and_context(struct Coord3d *pos, unsigned char *context)
{
  ...
  if ((kfx_render_state.pointer_x < 0) || (kfx_render_state.pointer_y < 0)
   ...
   || (kfx_render_state.pointer_y >= local_info.engine_window_height/pixel_size))
      return false;
  if (kfx_render_state.top_pointed_at_x <= kfx_sim_state.map_subtiles_x)
    x = kfx_render_state.top_pointed_at_x;
  else
    x = kfx_sim_state.map_subtiles_x;
  if (kfx_render_state.top_pointed_at_y <= kfx_sim_state.map_subtiles_y)
  ...
  if (pos->x.val >= (kfx_sim_state.map_subtiles_x << 8))
    pos->x.val = (kfx_sim_state.map_subtiles_x << 8)-1;
  if (pos->y.val >= (kfx_sim_state.map_subtiles_y << 8))
    pos->y.val = (kfx_sim_state.map_subtiles_y << 8)-1;
  return true;
}
...
static void get_dungeon_control_nonaction_inputs(void)
{
  ...
  {
    if (screen_to_map(get_local_camera(get_player_active_camera(player)), kfx_game_state.my_mouse_x, kfx_game_state.my_mouse_y, &pos))
    {
...
static void get_map_nonaction_inputs(void)
{
    ...
    if (((kfx_sim_state.operation_flags & GOF_Paused) == 0) && (player->view_mode == PVM_ParchmentView))
    {
        get_overhead_view_nonaction_inputs();
    }
}
...
static short get_inputs(void)
{
    ...
    if ((player->allocflags & PlaF_MouseInputDisabled) != 0)
    {
        ...
        if ((!game_is_busy_doing_gui_string_input()) && ((kfx_sim_state.operation_flags & GOF_Paused) != 0))
        {
    ...
    if (!flag_is_set(kfx_sim_state.operation_flags,GOF_Paused) || active_menu_functions_while_paused() || flag_is_set(kfx_sim_state.operation_flags,GOF_WorldInfluence))
...
static void process_cheat_mode_selection_inputs(void)
{
    ...
    switch (player->work_state)
    {
        ...
        case PSt_MkDigger:
        {
            ...
            else if (is_key_pressed(KC_LSHIFT, KMod_DONTCARE))
            {
                if (player->work_state == PSt_MkGoodCreatr)
                {
                    ...
                    {
                        new_value++;
                        if (new_value >= kfx_config_state.conf.crtr_conf.model_count)
                        {
                            new_value = 0;
                            break;
                        }
            ...
            else if (is_key_pressed(KC_LCONTROL, KMod_DONTCARE))
            {
                if (player->work_state == PSt_MkGoodCreatr)
                {
                    ...
                    do
                    {
                        if (new_value == 0)
                        {
                            new_value = kfx_config_state.conf.crtr_conf.model_count - 1;
                        }
                        else
                        {
                           new_value--;
        ...
        case PSt_PlaceTerrain:
        {
            ...
            {
                new_value++;
                if (new_value >= kfx_config_state.conf.slab_conf.slab_types_count)
                {
                    new_value = SlbT_ROCK;
                }
                set_players_packet_action(player, PckA_CheatSwitchTerrain, new_value, 0, 0, 0);
            ...
            else if (is_key_pressed(KC_LCONTROL, KMod_DONTCARE))
            {
                ...
                {
                    new_value = kfx_config_state.conf.slab_conf.slab_types_count - 1;
                }
                else
                {
                    new_value--;
...
{
    const char *statstr;
    toggle_flag(kfx_sim_state.mode_flags, MFlg_NoHeroHealthFlower);
    if (kfx_sim_state.mode_flags & MFlg_NoHeroHealthFlower)
    {
      statstr = "off";
    } else
    {
    ...
    show_onscreen_msg(2*kfx_sim_state.turns_per_second, "Hero health flowers %s", statstr);
}
...
void update_time(void)
{
    ...
    time /= 60;
    kfx_sim_state.Timer.Minutes = time % 60;
