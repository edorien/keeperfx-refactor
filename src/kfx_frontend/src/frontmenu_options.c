...
{
    gbtn->flags ^= (gbtn->flags ^ LbBtnF_Enabled * (kfx_frontend_state.define_key_scroll_offset != 0)) & LbBtnF_Enabled;
        if ((wheel_scrolled_up || is_key_pressed(KC_UP,KMod_NONE)) & (kfx_frontend_state.define_key_scroll_offset != 0))
    {
        kfx_frontend_state.define_key_scroll_offset--;
    }
}

void frontend_define_key_down_maintain(struct GuiButton *gbtn)
{
    gbtn->flags ^= (gbtn->flags ^ LbBtnF_Enabled * (kfx_frontend_state.define_key_scroll_offset < num_definable_keys()-1)) & LbBtnF_Enabled;
    if ((wheel_scrolled_down || is_key_pressed(KC_DOWN,KMod_NONE)) & (kfx_frontend_state.define_key_scroll_offset < num_definable_keys()-(frontend_define_keys_menu_items_visible-1)))
    {
        kfx_frontend_state.define_key_scroll_offset++;
    }
}
...
{
    if (kfx_frontend_state.define_key_scroll_offset > 0) {
        kfx_frontend_state.define_key_scroll_offset--;
    }
}
...
{
    if (kfx_frontend_state.define_key_scroll_offset < num_definable_keys()-(frontend_define_keys_menu_items_visible-1)) {
        kfx_frontend_state.define_key_scroll_offset++;
    }
}
...
{
    kfx_frontend_state.define_key_scroll_offset = frontend_scroll_tab_to_offset(gbtn, GetMouseY(), frontend_define_keys_menu_items_visible-2, num_definable_keys());
}
