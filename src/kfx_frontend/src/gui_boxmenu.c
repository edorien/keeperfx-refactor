...
// Boxes used for service/cheat menu -- moved to kfx_frontend_state.h (stage 10)
...
void gui_draw_box(struct GuiBox *gbox)
{
    ...
    if (gbox != gui_get_highest_priority_box())
    {
        ...
        RendererSetDrawColour(kfx_sim_state.colours[3][3][3]);
        ...
        while (goptn->label[0] != '!')
        {
          if (goptn->active_cb != NULL)
            ...
            goptn->enabled = 1;
          if (!goptn->enabled)
            RendererSetDrawColour(kfx_sim_state.colours[0][0][0]);
          else
            RendererSetDrawColour(kfx_sim_state.colours[3][3][3]);
          if (LbScreenIsLocked())
          {
    ...
    } else
    {
        ...
        while (goptn->label[0] != '!')
        {
            ...
            if ( ((gbox == gbox_over) && (goptn == goptn_over) && (gbox != dragging_box.gbox)) ||
              ...
              RendererSetDrawColour(kfx_sim_state.colours[9][9][9]);
            if (LbScreenIsLocked())
            {
              LbTextDraw(pos_x/pixel_size, pos_y/pixel_size, goptn->label);
            }
