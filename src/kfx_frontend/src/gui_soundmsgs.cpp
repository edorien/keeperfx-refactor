                                       ...
                                       std::string& found_lang)
{
    ...
    // speech/eng/file.ogg etc. (same fallback order as above)
	{
		...
		if (lang != nullptr && lang[0] != '\0') {
			...
			std::string extracted = extract_zip_speech_to_temp(kfx_render_state.last_level, lang_path);
			if (!extracted.empty()) {
		...
		if (lang == nullptr || strcmp(lang, "eng") != 0) {
			...
			std::string extracted = extract_zip_speech_to_temp(kfx_render_state.last_level, eng_path);
			if (!extracted.empty()) {
				result = SpeechResolveResult::ZipBundled;
				return extracted;
			}
		}
		{
			...
			std::string extracted = extract_zip_speech_to_temp(kfx_render_state.last_level, base_path);
			if (!extracted.empty()) {
				result = SpeechResolveResult::ZipBundled;
				return extracted;
			}
