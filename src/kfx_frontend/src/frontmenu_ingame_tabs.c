...
void gui_area_big_room_button(struct GuiButton *gbtn)
{
    ...
    {
        if ((player->work_state == PSt_BuildRoom) && (ustate->chosen_room_kind == kfx_sim_state.chosen_room_kind)
...
void choose_workshop_item(int manufctr_idx, TextStringId tooltip_id)
{
    ...
    kfx_sim_state.manufactr_element = manufctr_idx;
    kfx_sim_state.manufactr_spridx = manufctr->bigsym_sprite_idx;
    kfx_sim_state.manufactr_tooltip = tooltip_id;
}
...
void gui_go_to_next_trap(struct GuiButton *gbtn)
{
    ...
    kfx_sim_state.manufactr_element = manufctr_idx;
    kfx_sim_state.manufactr_spridx = manufctr->bigsym_sprite_idx;
    kfx_sim_state.manufactr_tooltip = manufctr->tooltip_stridx;
}
...
void gui_area_trap_button(struct GuiButton *gbtn)
{
     ...
     !is_trap_built(my_player_number, manufctr->tngmodel)))
    {
        ...
        } else if (kfx_sim_state.manufactr_element == manufctr_idx)
        {
...
void maintain_big_spell(struct GuiButton *gbtn)
{
    ...
    gbtn->sprite_idx = kfx_sim_state.chosen_spell_spridx;
    gbtn->tooltip_stridx = kfx_sim_state.chosen_spell_tooltip;
    struct Dungeon* dungeon = get_players_num_dungeon(my_player_number);
...
void maintain_big_room(struct GuiButton *gbtn)
{
    ...
    gbtn->sprite_idx = kfx_sim_state.chosen_room_spridx;
    gbtn->tooltip_stridx = kfx_sim_state.chosen_room_tooltip;
    if (dungeon->room_buildable[rkind] & 1) {
        gbtn->btype_value &= LbBFeF_IntValueMask;
        gbtn->flags |= LbBtnF_Enabled;
    } else {
...
void maintain_big_trap(struct GuiButton *gbtn)
{
    ...
    gbtn->tooltip_stridx = kfx_sim_state.manufactr_tooltip;
    if ( ((manufctr->tngclass == TCls_Trap) && is_trap_placeable(my_player_number, manufctr->tngmodel))
      || ((manufctr->tngclass == TCls_Door) && is_door_placeable(my_player_number, manufctr->tngmodel)) )
    {
        gbtn->btype_value &= LbBFeF_IntValueMask;
...
void gui_go_to_next_room(struct GuiButton *gbtn)
{
    ...
    kfx_sim_state.chosen_room_tooltip = gbtn->tooltip_stridx;
}
...
void maintain_activity_up(struct GuiButton *gbtn)
{
    ...
    {
        gbtn->flags |= LbBtnF_Visible;
        gbtn->flags ^= (gbtn->flags ^ LbBtnF_Enabled * (kfx_frontend_state.top_of_breed_list > 0)) & LbBtnF_Enabled;
    }
    if (wheel_scrolled_up && ((is_game_key_pressed(Gkey_SpeedMod, false, true)) || mouse_is_over_side_panel_bottom()))
    {
        if (kfx_frontend_state.top_of_breed_list > 0)
        {
            kfx_frontend_state.top_of_breed_list--;
        }
    }
}
...
void maintain_activity_down(struct GuiButton *gbtn)
{
    ...
    {
        gbtn->flags |= LbBtnF_Visible;
        gbtn->flags ^= (gbtn->flags ^ LbBtnF_Enabled * (kfx_frontend_state.no_of_breeds_owned - 6 > kfx_frontend_state.top_of_breed_list)) & LbBtnF_Enabled;
    }
    if (wheel_scrolled_down && (is_game_key_pressed(Gkey_SpeedMod, false, true) || mouse_is_over_side_panel_bottom()))
    {
        if (kfx_frontend_state.top_of_breed_list + 6 < kfx_frontend_state.no_of_breeds_owned)
        {
            kfx_frontend_state.top_of_breed_list++;
        }
    }
...
void maintain_event_button(struct GuiButton *gbtn)
{
    ...
    if ((event->kind == EvKind_Objective) && (kfx_sim_state.new_objective))
    {
        activate_event_box(evidx);
    }
    if (((event->kind == EvKind_FriendlyFight) || (event->kind == EvKind_EnemyFight))
        && ((event->mappos_x != 0) || (event->mappos_y != 0)) && ((get_gameturn() % (2 * kfx_config_state.gui_blink_rate)) >= kfx_config_state.gui_blink_rate))
    {
...
void draw_whole_status_panel(void)
{
    ...
    int mm_units_per_px;
    {
        ...
        if (mm_units_per_px < 1)
            mm_units_per_px = 1;
        fs_units_per_px = (gmnu->height * 16 + 8) / LbTiledSpriteHeight(&status_panel, get_panel_sprite);
    }
...
void update_powers_tab_to_config(void)
{
    ...
    for (PowerKind pwkind = PwrK_None; pwkind < kfx_config_state.conf.magic_conf.power_types_count; pwkind++)
    {
        struct PowerConfigStats* powerst = get_power_model_stats(pwkind);
...
void update_creatr_model_activities_list(TbBool forced)
{
    ...
    for (crmodel = 1; crmodel < kfx_config_state.conf.crtr_conf.model_count; crmodel++)
    {
        ...
        kfx_frontend_state.no_of_breeds_owned = num_breeds;
    }
