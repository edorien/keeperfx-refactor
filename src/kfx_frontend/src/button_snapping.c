...
static TbBool try_scroll_select_list_edge(long mouse_x, long mouse_y, float dx, float dy)
{
    ...
    for (int i = 0; i < ACTIVE_BUTTONS_COUNT; i++) {
        ...
        if (gbtn->click_event == frontend_mappack_select) {
            ...
            first_row = FE_LEVEL_SELECTLIST_ROW_BASE;
            last_row = FE_LEVEL_SELECTLIST_ROW_BASE + frontend_select_level_items_max_visible - 1;
        }
        if (is_down && gbtn->content.lval != last_row) {
            return false;
        }
        if (is_up && gbtn->content.lval != first_row) {
            return false;
        }
