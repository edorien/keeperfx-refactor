...
static inline TbBool update_gui_tooltip_target(void *target)
{
    ...
    {
        kfx_frontend_state.help_tip_time = 0;
...
{
    kfx_frontend_state.tool_tip_time = 0;
...
TbBool setup_trap_tooltips(struct Coord3d *pos)
{
    ...
    if ((kfx_frontend_state.help_tip_time > 20) || (player->work_state == PSt_CreatrQuery))
    {
        ...
        if (objst->tooltip_stridx == -1)
        {
            ...
            if (thing_is_special_box(thing))
            {
                if (thing_is_custom_special_box(thing))
                {
                    ...
                    {
                        set_gui_tooltip_box_fmt(5, "%s", kfx_sim_state.box_tooltip[thing->custom_box.box_kind]);
                    }
                }
                else
                {
...
short setup_room_tooltips(struct Coord3d *pos)
{
  ...
  if (in_query_mode == false) {
      ...
      if (kfx_frontend_state.help_tip_time <= 50) {
          kfx_frontend_state.help_tip_time++;
          return true;
      }
  }
...
void setup_gui_tooltip(struct GuiButton* gbtn)
{
    ...
    if (i == GUIStr_PickCreatrMostExpDesc)
    {
        ...
        if ((k > 0) && (kfx_frontend_state.top_of_breed_list + k < kfx_config_state.conf.crtr_conf.model_count))
            k = breed_activities[kfx_frontend_state.top_of_breed_list + k];
        else
...
TbBool gui_button_tooltip_update(int gbtn_idx)
{
  ...
  if ((get_active_menu(gbtn->gmenu_idx)->visual_state == 2) && ((gbtn->btype_value & LbBFeF_NoTooltip) == 0))
  {
    if (tool_tip_box.gbutton == gbtn)
    {
        ...
        if ( (kfx_frontend_state.tool_tip_time > tooltip_delay) || (player->work_state == PSt_CreatrQuery) )
        {
        ...
        {
          kfx_frontend_state.tool_tip_time++;
          busy_doing_gui = 1;
        }
    } else
    {
...
TbBool input_gameplay_tooltips(TbBool gameplay_on)
{
    ...
    TbBool shown = false;
    struct PlayerInfo* player = get_my_player();
    if ((gameplay_on) && (kfx_frontend_state.tool_tip_time == 0) && (!busy_doing_gui))
    {
...
void toggle_tooltips(void)
{
  ...
  show_onscreen_msg(2*kfx_sim_state.turns_per_second, "Tooltips %s", statstr);
  save_settings();
}
    ...
    if (ttwidth > viswidth)
    {
        if (render_tooltip_scroll_timer <= 0)
        {
            if (-ttwidth >= render_tooltip_scroll_offset)
              ...
              render_tooltip_scroll_offset -= ((MyScreenHeight >= 400) ? 4.0 : 2.0) * kfx_render_state.delta_time;
        } else
        {
            render_tooltip_scroll_timer -= 1.0 * kfx_render_state.delta_time;
            if (render_tooltip_scroll_timer < 0)
              render_tooltip_scroll_offset = 0;
        }
    }
...
{
    SYNCDBG(7,"Starting");
    // Phase 3: with the ImGui in-game HUD active, ingame_tooltip_frame()
    ...
    // and populates tool_tip_box, only this final draw moves. Only reached
