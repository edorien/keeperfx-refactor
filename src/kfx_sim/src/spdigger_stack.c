        ...
        do
        {
            ...
            if (steps_per_direction > 0)
            {
                while (1)
                {
                    slb_x += ar->delta_x;
                    slb_y += ar->delta_y;
                    if (slb_x >= 0 && slb_x < kfx_sim_state.map_tiles_x && slb_y >= 0 && slb_y < kfx_sim_state.map_tiles_y && check_place_to_reinforce(thing, slb_x, slb_y) > 0)
                    {
...
TbBool thing_can_be_picked_to_place_in_player_room_of_role(const struct Thing* thing, PlayerNumber plyr_idx, RoomRole rrole, unsigned short flags)
{
    ...
    if ((thing->owner == kfx_config_state.neutral_player_num) && ((slabmap_owner(slb) == kfx_config_state.neutral_player_num) || (slabmap_owner(slb) == dungeon->owner)))
    {
        return true;
    } else
        ...
        if (thing_is_object(thing) && object_is_gold_pile(thing))
        {
            ...
            if (!thing_is_picked_up(thing) && !thing_is_dragged_or_pulled(thing))
            {
                  if (thing_revealed(thing, owner))
                  {
                      ...
                      if ((slb_owner == owner) || (slb_owner == kfx_config_state.neutral_player_num)) {
                          struct Room *room;
                          room = find_any_navigable_room_for_thing_closer_than(thing, owner, RoRoF_GoldStorage, NavRtF_Default, kfx_sim_state.map_subtiles_x/2 + kfx_sim_state.map_subtiles_y/2);
    ...
    while (i != 0)
    {
        ...
        if ((dungeon->owner == thing->owner) && creature_is_being_unconscious(thing) && !thing_is_dragged_or_pulled(thing))
        {
            if (thing_revealed(thing, dungeon->owner))
            {
                ...
                if (kfx_config_state.conf.rules[dungeon->owner].workers.drag_to_lair == 1)
                {
                ...
                else if (kfx_config_state.conf.rules[dungeon->owner].workers.drag_to_lair == 2)
                {
...
long check_out_worker_save_unconscious(struct Thing *thing, struct DiggerStack *dstack)
{
    ...
    stl_x = stl_num_decode_x(dstack->stl_num);
    stl_y = stl_num_decode_y(dstack->stl_num);
    if(!kfx_config_state.conf.rules[thing->owner].workers.drag_to_lair)
    {
        return 0;
    }
    struct Thing *sectng;
    ...
    if (room_is_invalid(room) && creature_can_do_healing_sleep(sectng) && kfx_config_state.conf.rules[thing->owner].workers.drag_to_lair == 2)
    {
