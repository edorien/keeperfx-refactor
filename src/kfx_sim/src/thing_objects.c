...
void set_call_to_arms_graphics(PlayerNumber plyr_idx, int birth_anim_idx, int alive_anim_idx, int leave_anim_idx)
{
    ...
    call_to_arms_graphics[plyr_idx].leave_anim_idx = leave_anim_idx;
}
...
void destroy_food(struct Thing *foodtng)
{
    ...
    if (kfx_config_state.neutral_player_num != plyr_idx) {
...
PowerKind book_thing_to_power_kind(const struct Thing *thing)
{
    ...
    if ( (thing->class_id != TCls_Object) || (thing->model >= kfx_config_state.conf.object_conf.object_types_count) )
        return 0;
    return kfx_config_state.conf.object_conf.object_to_power_artifact[thing->model];
}
...
void update_all_objects_of_model(ThingModel model)
{
    ...
    for (int i = slist->index; i > 0;)
    {
        ...
        if (thing->owner != kfx_config_state.neutral_player_num)
        {
            ...
            {
                try_set_backup_heart_idx(thing->owner, thing->index);
            }
        }
...
static long food_moves(struct Thing *objtng)
{
    ...
    if (!dirct_ctrl)
    {
      ...
      else
      {
            ...
            {
                objtng->food.life_remaining = kfx_config_state.conf.rules[objtng->owner].gameplay.food_life_out_of_hatchery;
      ...
      if (objtng->food.life_remaining >= 0)
      {
        ...
        if (objtng->food.life_remaining <= 0)
        {
            if (objtng->owner != kfx_config_state.neutral_player_num)
            {
...
static TngUpdateRet object_update_dungeon_heart(struct Thing *heartng)
{
    ...
    if ((heartng->health > 0) && (kfx_config_state.conf.rules[heartng->owner].gameplay.dungeon_heart_heal_time != 0))
    {
        if ((get_gameturn() % kfx_config_state.conf.rules[heartng->owner].gameplay.dungeon_heart_heal_time) == 0)
        {
            heartng->health += kfx_config_state.conf.rules[heartng->owner].gameplay.dungeon_heart_heal_health;
            if (heartng->health < 0)
            {
              heartng->health = 0;
            } else
...
GoldAmount gold_object_typical_value(struct Thing *thing)
{
    ...
    {
      case ObjMdl_GoldChest:
          return kfx_config_state.conf.rules[thing->owner].gameplay.chest_gold_hold;
      case ObjMdl_GoldPot:
          return kfx_config_state.conf.rules[thing->owner].gameplay.pot_of_gold_holds;
      case ObjMdl_Goldl:
          return kfx_config_state.conf.rules[thing->owner].gameplay.gold_pile_value;
      case ObjMdl_GoldBag:
          return kfx_config_state.conf.rules[thing->owner].gameplay.bag_gold_hold;
      case ObjMdl_SpinningCoin:
          return kfx_config_state.conf.rules[thing->owner].gameplay.gold_pile_maximum;
      default:
        break;
    }
    return 0;
...
struct Thing *drop_gold_pile(long value, struct Coord3d *pos)
{
    ...
    if (thing_is_invalid(thing)) {
        thing = create_gold_pile(pos, kfx_config_state.neutral_player_num, value);
    } else {
