...
long reveal_players_map_to_player(struct Thing *thing, PlayerNumber benefit_plyr_idx)
{
    ...
    unsigned char* ownership_map = (unsigned char*)malloc(kfx_sim_state.map_tiles_y * kfx_sim_state.map_tiles_x);
    memset(ownership_map,0,kfx_sim_state.map_tiles_y*kfx_sim_state.map_tiles_x);
...
CrCheckRet process_torture_function(struct Thing *creatng)
{
    ...
    if ((kfx_config_state.conf.rules[room->owner].gameplay.classic_bugs_flags & ClscBug_NeutralTortureConverts) == 0)
    {
        if (room->owner == kfx_config_state.neutral_player_num || is_neutral_thing(creatng))
        {
            return CrCkRet_Available;
        }
    }
    ...
    {
        if (THING_RANDOM(creatng, 100) < kfx_config_state.conf.rules[room->owner].rooms.ghost_convert_chance)
        {
