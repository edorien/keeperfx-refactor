...
extern "C" {
#endif
// Duplicated from kfx_game's lvl_script.h (CurrentPlayer) rather than
// included, to avoid a kfx_sim -> kfx_game layering violation for a
// single stable sentinel value. See
// docs/refactor/stage-13-enforce-and-document.md.
#define MAP_LOCATIONS_CURRENT_PLAYER 15
/******************************************************************************/
...
TbBool get_coords_at_meta_action(struct Coord3d *pos, PlayerNumber target_plyr_idx, long i)
{
    ...
    if (loc_player == 15) // CURRENT_PLAYER
        loc_player = kfx_sim_state.script_current_player;
    ...
    {
    case MML_LAST_EVENT:
        src = &kfx_sim_state.triggered_object_location;
        break;
