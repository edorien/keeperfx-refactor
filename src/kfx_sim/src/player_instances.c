...
long pinstfe_hand_whip(struct PlayerInfo *player, int32_t *n)
{
  ...
  case TCls_Shot:
      ...
      if (shotst->model_flags & ShMF_Boulder)
      {
          ...
          {
              thing->health -= kfx_config_state.conf.rules[thing->owner].gameplay.boulder_reduce_health_slap;
          }
      }
      else
      {
...
long pinstfs_zoom_to_heart(struct PlayerInfo *player, int32_t *n)
{
    ...
    sim_feedback->light_turn_light_off(player->cursor_light_idx);
...
long pinstfs_zoom_out_of_heart(struct PlayerInfo *player, int32_t *n)
{
    ...
    if (!thing_exists(thing))
    {
        cam->mappos.x.val = subtile_coord_center(kfx_sim_state.map_subtiles_x / 2);
        cam->mappos.y.val = subtile_coord_center(kfx_sim_state.map_subtiles_y / 2);
        cam->zoom = 24000;
  ...
  kfx_sim_state.TimerFreeze = false;
  return 0;
}
