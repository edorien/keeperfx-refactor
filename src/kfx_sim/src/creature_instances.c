...
{
    return (inst_inf < &kfx_config_state.conf.magic_conf.instance_info[1]);
}
...
TbBool creature_increase_available_instances(struct Thing *thing)
{
    ...
    for (int i = 0; i < LEARNED_INSTANCES_COUNT; i++)
    {
        ...
        if (k > 0)
        {
            ...
            else if ( (crconf->learned_instance_level[i] > cctrl->exp_level+1) && !(kfx_config_state.conf.rules[thing->owner].gameplay.classic_bugs_flags & ClscBug_RebirthKeepsSpells) )
            {
...
long instf_destroy(struct Thing *creatng, int32_t *param)
{
    ...
    if ( !room_is_invalid(room) && (prev_owner != creatng->owner) )
    {
        ...
        {
            remove_room_from_players_list(room, kfx_config_state.neutral_player_num);
