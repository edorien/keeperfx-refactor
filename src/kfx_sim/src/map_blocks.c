...
{
    if ( (plyr_idx == kfx_config_state.neutral_player_num) || subtile_revealed_directly(slab_subtile_center(slb_x), slab_subtile_center(slb_y), plyr_idx) )
    {
        return false;
    }
...
void neutralise_enemy_block(MapSubtlCoord stl_x, MapSubtlCoord stl_y, PlayerNumber domn_plyr_idx)
{
    ...
    {
    case 1:
        place_slab_type_on_map(SlbT_LAVA,  slab_subtile(slb_x,0), slab_subtile(slb_y,0), kfx_config_state.neutral_player_num, 0);
        break;
    case 2:
        place_slab_type_on_map(SlbT_WATER, slab_subtile(slb_x,0), slab_subtile(slb_y,0), kfx_config_state.neutral_player_num, 0);
        break;
    default:
        place_slab_type_on_map(SlbT_PATH,  slab_subtile(slb_x,0), slab_subtile(slb_y,0), kfx_config_state.neutral_player_num, 1);
        break;
    }
...
{
    struct Column *col;
    col = &kfx_sim_state.columns_data[col_idx];
    memcpy(col, &kfx_sim_state.columns_data[0], sizeof(struct Column));
    col->use = 0;
}
...
void copy_block_with_cube_groups(short itm_idx, MapSubtlCoord stl_x, MapSubtlCoord stl_y)
{
    ...
    struct Column col;
    memcpy(&col, &kfx_sim_state.columns_data[-itm_idx], sizeof(struct Column));
    col.use = 0;
    col.bitfields &= ~0x01;
    TbBool found;
    found = false;
    ...
    for (i=0; i < COLUMN_STACK_HEIGHT; i++)
    {
        if (col.cubes[i] > 0)
        {
            ...
            if (cubed->ownershipGroup > 0) {
                ...
                col.cubes[i] = kfx_config_state.conf.cube_conf.cube_bits[cubed->ownershipGroup][get_player_color_idx(slabmap_owner(slb))];
            }
        }
    }
    if (found) {
...
void place_slab_object(SlabCodedCoords slb_num, MapSubtlCoord stl_x,MapSubtlCoord stl_y, unsigned short slabset_id, unsigned short stl_id, PlayerNumber plyr_idx)
{
    ...
    for (; sobj_idx < kfx_sim_state.slabobjs_num; sobj_idx++)
    {
        struct SlabObj *sobj;
        sobj = &kfx_sim_state.slabobjs[sobj_idx];
...
void delete_attached_lights_on_slab(MapSlabCoord slb_x, MapSlabCoord slb_y)
{
    ...
    if (start_stl_y <= 0)
      start_stl_y = 0;
    end_stl_x = 3 * slb_x + 4;
    if (end_stl_x >= kfx_sim_state.map_subtiles_x)
      end_stl_x = kfx_sim_state.map_subtiles_x;
    end_stl_y = 3 * slb_y + 4;
            ...
            (slabsta->category == SlbAtCtg_Obstacle) || (slb->kind == SlbT_WATER) || (slb->kind == SlbT_LAVA))
        {
            ...
            if (slabstb->category == SlbAtCtg_FortifiedWall)
            {
                ...
                if (slabstc->category == SlbAtCtg_FortifiedWall)
                {
                  ...
                  struct SlabSet *sset;
                  sset = &kfx_sim_state.slabset[slabset_id];
...
void place_single_slab_type_on_map(SlabKind slbkind, MapSlabCoord slb_x, MapSlabCoord slb_y, PlayerNumber plyr_idx)
{
    ...
    {
        struct SlabMap *slb = get_slabmap_block(slb_x,slb_y);
        slb->health = kfx_sim_state.block_health[place_slabst->block_health_index];
    }
...
void dump_slab_on_map(SlabKind slbkind, long slabset_id, MapSubtlCoord stl_x, MapSubtlCoord stl_y, PlayerNumber owner)
{
    ...
    struct SlabMap *slb;
    slb = get_slabmap_block(slb_x, slb_y);
    slb->health = kfx_sim_state.block_health[slabst->block_health_index];
    struct SlabSet *sset;
    sset = &kfx_sim_state.slabset[slabset_id];
    ...
    if (slab_kind_is_animated(slbkind) && !slab_kind_is_door(slbkind))
    {
        ...
        if (stl_yb > kfx_sim_state.map_subtiles_y)
            stl_yb = kfx_sim_state.map_subtiles_y;
        stl_xb = stl_xa + STL_PER_SLB - 1;
        if (stl_xb > kfx_sim_state.map_subtiles_x)
            stl_xb = kfx_sim_state.map_subtiles_x;
...
void place_slab_type_on_map_f(SlabKind nslab, MapSubtlCoord stl_x, MapSubtlCoord stl_y, PlayerNumber owner, unsigned char keep_blocks_around,const char *func_name)
{
    ...
    for (i = 0; i < AROUND_EIGHT_LENGTH; i++)
    {
        ...
        if ((previous_slab_types_around[i] != slb->kind)
          ...
          || (kfx_sim_state.game_kind == GKind_NonInteractiveState))
        {
...
void replace_map_slab_when_destroyed(MapSlabCoord slb_x, MapSlabCoord slb_y)
{
    ...
    place_slab_type_on_map(nslab, slab_subtile(slb_x,0), slab_subtile(slb_y,0), kfx_config_state.neutral_player_num, 0);
}
...
void create_gold_rubble_for_dug_slab(MapSlabCoord slb_x, MapSlabCoord slb_y)
{
    ...
    for (y = stl_y; y < stl_y+STL_PER_SLB; y++)
    {
        for (x = stl_x; x < stl_x+STL_PER_SLB; x++)
        {
            ...
            if (z > 0)
            {
                create_gold_rubble_for_dug_block(x, y, z, kfx_config_state.neutral_player_num);
            }
        }
    }
}
...
void create_dirt_rubble_for_dug_slab(MapSlabCoord slb_x, MapSlabCoord slb_y)
{
    ...
    for (y = stl_y; y < stl_y+STL_PER_SLB; y++)
    {
        for (x = stl_x; x < stl_x+STL_PER_SLB; x++)
        {
            ...
            if (z > 0)
            {
                create_dirt_rubble_for_dug_block(x, y, z, kfx_config_state.neutral_player_num);
            }
        }
    }
}
...
void reinit_tagged_blocks_for_player(PlayerNumber plyr_idx)
{
    ...
    MapSubtlCoord stl_x;
    MapSubtlCoord stl_y;
    for (stl_y=0; stl_y < kfx_sim_state.map_subtiles_y; stl_y++)
    {
        for (stl_x=0; stl_x < kfx_sim_state.map_subtiles_x; stl_x++)
        {
            struct Map *mapblk;
            mapblk = get_map_block_at(stl_x, stl_y);
            mapblk->flags &= ~SlbAtFlg_Unexplored;
