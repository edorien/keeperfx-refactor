...
// Real usage: set_chosen_power_none() (add_power_to_player()/
// remove_power_from_player(), moved from config_magic.c).
    ...
    for (shift_y=0; shift_y < 2*MAX_SOE_RADIUS; shift_y++)
    {
        ...
        for (shift_x = 0; shift_x <= MAX_SOE_RADIUS; shift_x++)
        {
          if (dungeon->soe_explored_flags[shift_y][shift_x])
          {
            ...
            if (stl_x_beg > kfx_sim_state.map_subtiles_x-1) {
                stl_x_beg = kfx_sim_state.map_subtiles_x-1;
            }
            if (stl_x_end < 0) {
                stl_x_end = 0;
            } else
            if (stl_x_end > kfx_sim_state.map_subtiles_x-1) {
                stl_x_end = kfx_sim_state.map_subtiles_x-1;
            }
    ...
    for (shift_x = 0; shift_x < 2*MAX_SOE_RADIUS; shift_x++)
    {
      ...
      for (shift_y = 0; shift_y <= MAX_SOE_RADIUS; shift_y++)
      {
        if (dungeon->soe_explored_flags[shift_y][shift_x])
        {
            ...
            if (stl_y_end > kfx_sim_state.map_subtiles_y-1) {
                stl_y_end = kfx_sim_state.map_subtiles_y-1;
            }
            if (stl_y_beg < 0) {
                stl_y_beg = 0;
            } else
            if (stl_y_beg > kfx_sim_state.map_subtiles_y-1) {
                stl_y_beg = kfx_sim_state.map_subtiles_y-1;
            }
...
TbBool can_cast_power_at_xy(PlayerNumber plyr_idx, PowerKind pwkind, MapSubtlCoord stl_x, MapSubtlCoord stl_y, unsigned long allow_flags)
{
    ...
    if ( ((mapblk->flags & SlbAtFlg_Blocking) != 0) && (!subtile_is_liquid_or_path) )
    {
        ...
        {
            if (slb_owner == kfx_config_state.neutral_player_num) {
                return true;
            }
        }
    ...
    } else
    {
        ...
        {
            if ((slabst->category != SlbAtCtg_Unclaimed) && (slb_owner == kfx_config_state.neutral_player_num)) {
                return true;
            }
        }
...
static TbResult magic_use_power_armageddon(PowerKind power_kind, PlayerNumber plyr_idx, struct Thing *thing, MapSubtlCoord stl_x, MapSubtlCoord stl_y, KeepPwrLevel power_level, unsigned long mod_flags)
{
    ...
    kfx_sim_state.armageddon_cast_turn = get_gameturn();
    kfx_sim_state.armageddon_caster_idx = plyr_idx;
    struct Thing *heartng;
    heartng = get_player_soul_container(plyr_idx);
    kfx_sim_state.armageddon_mappos.x.val = heartng->mappos.x.val;
    ...
    kfx_sim_state.armageddon_over_turn = kfx_config_state.conf.rules[plyr_idx].magic.armageddon_duration + enemy_time_gap;
    ...
    for (MapSlabCoord slb_y=slb_y_start; slb_y < slb_y_end ; slb_y++)
    {
        for (MapSlabCoord slb_x=slb_x_start; slb_x < slb_x_end ; slb_x++,i++)
        {
            ...
            if (destreff == 79)
            {
                ...
                {
                    place_slab_type_on_map(SlbT_EARTH, slab_subtile_center(slb_x),slab_subtile_center(slb_y), kfx_config_state.neutral_player_num, 0);
...
static void magic_power_hold_audience_update(PlayerNumber plyr_idx)
{
    ...
    dungeon = get_players_num_dungeon(plyr_idx);
    SYNCDBG(8,"Starting");
    if ( get_gameturn() - dungeon->hold_audience_cast_turn <= kfx_config_state.conf.rules[plyr_idx].magic.hold_audience_time) {
        return;
    }
...
void process_magic_power_call_to_arms(PlayerNumber plyr_idx)
{
    ...
    if (!free)
    {
        if ((kfx_config_state.conf.rules[plyr_idx].gameplay.allies_share_cta) && (players_are_mutual_allies(plyr_idx, slabmap_owner(slb))))
        {
            free = true;
        }
    }
...
void process_dungeon_power_magic(void)
{
    ...
    for (i = 0; i < PLAYERS_COUNT; i++)
    {
        ...
        if (player_exists(player))
        {
            ...
            if (kfx_sim_state.armageddon_cast_turn > 0)
            {
                if (get_gameturn() > kfx_sim_state.armageddon_over_turn)
                {
...
/**
 * Grants a keeper power to a player (increments its magic_level).
 * Moved from config_magic.c -- this is the live runtime operation, not
...
/**
 * Revokes a keeper power from a player (decrements its magic_level and
 * turns off any active effect). Moved from config_magic.c -- see
 * add_power_to_player() above.
 */
void remove_power_from_player(PowerKind pwkind, PlayerNumber plyr_idx)
{
    ...
    if (kfx_sim_state.chosen_spell_type == pwkind)
    {
