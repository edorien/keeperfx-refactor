...
void remove_body_from_graveyard(struct Thing *thing)
{
    ...
    {
        if (dungeon->bodies_rotten_for_vampire >= kfx_config_state.conf.rules[dungeon->owner].rooms.bodies_for_vampire) {
            dungeon->bodies_rotten_for_vampire -= kfx_config_state.conf.rules[dungeon->owner].rooms.bodies_for_vampire;
...
TngUpdateRet update_dead_creature(struct Thing *thing)
{
    ...
    if ((thing->alloc_flags & TAlF_IsDragged) == 0)
    {
        ...
        if (corpse_laid_to_rest(thing))
        {
            ...
            {
                if (get_gameturn() - thing->creation_turn > kfx_config_state.conf.rules[thing->owner].creature.body_remains_for) {
                    delete_thing_structure(thing, 0);
                    return TUFRet_Deleted;
                }
            }
        } else
        {
            ...
            if (((corpse_age > kfx_config_state.conf.rules[thing->owner].creature.body_remains_for) ||(!corpse_is_rottable(thing) && (corpse_age > VANISH_EFFECT_DELAY)))
...
struct Thing *create_dead_creature(const struct Coord3d *pos, ThingModel model, unsigned short crpscondition, unsigned short owner, CrtrExpLevel exp_level)
{
    ...
    case DCrSt_Dead:
        ...
        set_thing_draw(thing, k, 256, kfx_config_state.conf.crtr_conf.sprite_size, 0, 0, ODC_Default);
        break;
    case DCrSt_Dying:
    default:
        thing->active_state = DCrSt_Dying;
        k = get_creature_anim(thing, CGI_Scream);
        set_thing_draw(thing, k, 128, kfx_config_state.conf.crtr_conf.sprite_size, 0, 0, ODC_Default);
