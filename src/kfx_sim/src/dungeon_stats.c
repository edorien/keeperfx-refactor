...
unsigned long compute_dungeon_rooms_variety_score(long room_types, long total_area)
{
    if (room_types < 0)
        room_types = 0;
    if (room_types > kfx_config_state.conf.slab_conf.room_types_count)
        room_types = kfx_config_state.conf.slab_conf.room_types_count;
    if (total_area < 0)
        total_area = 0;
    if (total_area >= 512)
        total_area = 512;
...
TbBool update_dungeon_scores_for_player(struct PlayerInfo *player)
{
    ...
    }
    {
        ...
        max_manage_efficiency += 40 * compute_dungeon_rooms_variety_score(kfx_config_state.conf.slab_conf.room_types_count, INT32_MAX);
    }
    {
