...
CrStateRet creature_add_lair_to_room(struct Thing *creatng, struct Room *room)
{
    ...
    lairtng->lair.spr_size = kfx_config_state.conf.crtr_conf.sprite_size + (kfx_config_state.conf.crtr_conf.sprite_size * kfx_config_state.conf.crtr_conf.exp.size_increase_on_exp * cctrl->exp_level) / 100;
    lairtng->move_angle_xy = THING_RANDOM(creatng, DEGREES_360);
...
short creature_sleep(struct Thing *thing)
{
    ...
    if (kfx_config_state.conf.rules[thing->owner].creature.recovery_frequency > 0)
    {
        if (((get_gameturn() + thing->index) % kfx_config_state.conf.rules[thing->owner].creature.recovery_frequency) == 0)
        {
