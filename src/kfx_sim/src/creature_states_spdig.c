          ...
          && thing_revealed(thing, spdigtng->owner) && creature_is_being_unconscious(thing))
        {
            if ((range < 0) || get_chessboard_distance(&thing->mappos, &spdigtng->mappos) < range)
            {
                if (!imp_will_soon_be_working_at_excluding(spdigtng, thing->mappos.x.stl.num, thing->mappos.y.stl.num))
                {
                    ...
                    if (kfx_config_state.conf.rules[spdigtng->owner].workers.drag_to_lair == 1) {
                    ...
                    else if (kfx_config_state.conf.rules[spdigtng->owner].workers.drag_to_lair == 2 && !creature_can_do_healing_sleep(thing))
                    {
                        return 0;
                    }
    ...
    while (i > 0)
    {
        ...
        if (thing_is_object(thing) && !thing_is_picked_up(thing) && thing_revealed(thing, spdigtng->owner))
        {
            if (object_is_gold_pile(thing))
            {
                ...
                if ((slabmap_owner(slb) == spdigtng->owner) || (slabmap_owner(slb) == kfx_config_state.neutral_player_num))
                {
                    if ((range < 0) || get_chessboard_distance(&thing->mappos, &spdigtng->mappos) < range)
                    {
                        if (!imp_will_soon_be_working_at_excluding(spdigtng, thing->mappos.x.stl.num, thing->mappos.y.stl.num))
...
long digger_work_experience(struct Thing* spdigtng)
{
    ...
    {
        return kfx_config_state.conf.rules[spdigtng->owner].workers.digger_work_experience;
    }
    return 0;
}
...
short imp_reinforces(struct Thing *thing)
{
    ...
    if (kfx_config_state.conf.rules[thing->owner].workers.digger_work_experience != 0)
    {
...
short creature_save_unconscious_creature(struct Thing *thing)
{
    ...
    if (room_is_invalid(dstroom) && creature_can_do_healing_sleep(picktng) && kfx_config_state.conf.rules[thing->owner].workers.drag_to_lair == 2)
    {
...
short creature_drops_crate_in_workshop(struct Thing *thing)
{
    ...
    cratetng->owner = kfx_config_state.neutral_player_num;
    ...
    if (kfx_config_state.conf.rules[thing->owner].workers.digger_work_experience != 0)
    {
        cctrl->exp_points += kfx_config_state.conf.rules[thing->owner].workers.digger_work_experience;
        check_experience_upgrade(thing);
    }
    return 1;
}
...
short creature_drops_spell_object_in_library(struct Thing *creatng)
{
    ...
    spelltng->owner = kfx_config_state.neutral_player_num;
