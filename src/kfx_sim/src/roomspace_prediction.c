...
static TbBool prevent_local_dig_prediction(const struct Packet *pckt)
{
    ...
    if (sim_feedback->is_battle_creature_over_active()) {
        return true;
    }
    return false;
}
...
{
    return network_is_active() && !sim_feedback->get_packet_load_enable() && (sim_feedback->get_input_lag_turns() > 0);
}
...
void update_local_dig_tag_prediction(void)
{
    ...
    if ((pckt->control_flags & PCtr_LBtnAnyAction) == 0) {
        ...
        if ((local_dig_tag_prediction.last_packet_turn != 0) && (get_gameturn() - local_dig_tag_prediction.last_packet_turn > sim_feedback->get_input_lag_turns() + LOCAL_DIG_TAG_EXPIRY)) {
            memset(&local_dig_tag_prediction, 0, sizeof(local_dig_tag_prediction));
        }
        return;
    }
...
void update_local_dig_prediction_cursor_preview(void)
{
    ...
    const struct Packet *pckt = sim_feedback->get_history_packet(player->packet_num, get_gameturn());
    ...
    local_dig_render_roomspace_active = true;
    sim_feedback->tag_cursor_blocks_dig(&predicted_player, pckt, &local_dig_render_roomspace, stl_x, stl_y, true);
    sim_feedback->reset_box_lag_compensation();
}
/******************************************************************************/
#ifdef __cplusplus
