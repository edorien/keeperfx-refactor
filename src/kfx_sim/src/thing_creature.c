...
// FlwB_* for the same established pattern).
#define SOpr_SET      1
...
long check_for_first_person_barrack_party(struct Thing *grthing)
{
    ...
    while (i != 0)
    {
        ...
        if (thing->index != grthing->index) {
            ...
            n++;
            if (n >= kfx_config_state.conf.rules[grthing->owner].rooms.barrack_max_party_size || n >= GROUP_MEMBERS_COUNT)
            {
                break;
            }
        }
...
long creature_available_for_combat_this_turn(struct Thing *creatng)
{
    ...
    if ((creatng->owner == kfx_config_state.neutral_player_num) || ((cctrl->creature_control_flags & CCFlg_NoCompControl) != 0)) {
        return false;
    }
...
void food_eaten_by_creature(struct Thing *foodtng, struct Thing *creatng)
{
    ...
    } else
    {
        ...
        apply_health_to_thing_and_display_health(creatng, kfx_config_state.conf.rules[creatng->owner].health.food_health_gain);
...
CrInstance get_available_instance_with_spell_effect(const struct Thing *thing, unsigned long spell_flags)
{
    ...
    SpellKind spell_idx;
    for (CrInstance i = 0; i < kfx_config_state.conf.crtr_conf.instances_count; i++)
    {
    ...
    && (crconf->fear_stronger > 0))
    {
        if (!creature_under_spell_effect(thing, CSAfF_Fear))
        {
            ...
            if (is_thing_directly_controlled(thing))
            {
                ...
                {
                    char active_menu = kfx_sim_state.active_panel_mnu_idx;
...
TbBool clear_thing_spell_flags_f(struct Thing *thing, unsigned long spell_flags, const char *func_name)
{
    ...
    && (creature_under_spell_effect(thing, CSAfF_Flying)))
    {
        ...
        if (!flag_is_set(kfx_config_state.conf.crtr_conf.model[thing->model].model_flags, CMF_IsDiptera))
        {
            clear_flag(thing->movement_flags, TMvF_Flying);
        }
        cleared = true;
    ...
    if (cspell->duration == spconf->duration / 2)
    {
        ...
        if (thing_in_wall_at(thing, &pos))
        {
            ...
            switch(player->teleport_destination)
            {
                ...
                default:
                {
                    rkind = sim_feedback->get_zoom_key_room_order(player->teleport_destination);
                }
            }
            if (rkind > 0)
            {
    ...
    PlayerNumber def_plyr_idx, CrDeathFlags flags)
{
    ...
    if ((flags & CrDed_DiedInBattle) != 0)
    {
        ...
        if ((def_plyr_idx != -1) && (kfx_config_state.neutral_player_num != def_plyr_idx))
        {
            return inc_player_kills_counter(def_plyr_idx, victim);
        }
    }
    if ((cctrl->fighting_player_idx != -1) && (kfx_config_state.neutral_player_num != cctrl->fighting_player_idx))
    {
...
void throw_out_gold(struct Thing* thing, long amount)
{
    ...
    else //drop pots
    {
        ...
        num_pots_to_drop = ((amount + kfx_config_state.conf.rules[thing->owner].gameplay.pot_of_gold_holds - 1) / kfx_config_state.conf.rules[thing->owner].gameplay.pot_of_gold_holds);
...
struct Thing* cause_creature_death(struct Thing *thing, CrDeathFlags flags)
{
    ...
    remove_parent_thing_from_things_in_list(&kfx_sim_state.thing_lists[TngList_Shots],thing->index);
    ...
    if ((!flag_is_set(flags,CrDed_NotReallyDying)) || (flag_is_set(kfx_config_state.conf.rules[thing->owner].gameplay.classic_bugs_flags,ClscBug_ResurrectRemoved)))
    {
    ...
    {
        if (flag_is_set(kfx_sim_state.mode_flags, MFlg_DeadBackToPool))
        {
    ...
    if (!creature_model_bleeds(thing->model))
    {
        ...
        if ((kfx_sim_state.mode_flags & MFlg_DeadBackToPool) != 0)
            add_creature_to_pool(crmodel, 1);
...
struct Thing *kill_creature(struct Thing *creatng, struct Thing *killertng, PlayerNumber killer_plyr_idx, CrDeathFlags flags)
{
    ...
    {
        if (killertng->owner == kfx_config_state.neutral_player_num)
        {
            clear_flag(flags, CrDed_DiedInBattle);
        }
    }
    if (killer_plyr_idx == kfx_config_state.neutral_player_num)
    {
        clear_flag(flags, CrDed_DiedInBattle);
    }
    if (!thing_exists(creatng))
    ...
    if (thing_is_invalid(killertng) || (killertng->owner == kfx_config_state.neutral_player_num) || (killer_plyr_idx == kfx_config_state.neutral_player_num) || dungeon_invalid(dungeon))
    {
        return cause_creature_death(creatng, flags);
    }
...
void thing_fire_shot(struct Thing *firing, struct Thing *target, ThingModel shot_model, CrtrExpLevel shot_level, unsigned char hit_type)
{
    ...
    else
    {
        ...
        max_dexterity = crconf->dexterity + ((crconf->dexterity * cctrl->exp_level * kfx_config_state.conf.crtr_conf.exp.dexterity_increase_on_exp) / 100);

        pos1.x.val += distance_with_angle_to_coord_x((cctrl->shot_shift_x + (cctrl->shot_shift_x * kfx_config_state.conf.crtr_conf.exp.size_increase_on_exp * cctrl->exp_level) / 100), firing->move_angle_xy + DEGREES_90);
        pos1.y.val += distance_with_angle_to_coord_y((cctrl->shot_shift_x + (cctrl->shot_shift_x * kfx_config_state.conf.crtr_conf.exp.size_increase_on_exp * cctrl->exp_level) / 100), firing->move_angle_xy + DEGREES_90);
        pos1.x.val += distance_with_angle_to_coord_x((cctrl->shot_shift_y + (cctrl->shot_shift_y * kfx_config_state.conf.crtr_conf.exp.size_increase_on_exp * cctrl->exp_level) / 100), firing->move_angle_xy);
        pos1.y.val += distance_with_angle_to_coord_y((cctrl->shot_shift_y + (cctrl->shot_shift_y * kfx_config_state.conf.crtr_conf.exp.size_increase_on_exp * cctrl->exp_level) / 100), firing->move_angle_xy);
        pos1.z.val += (cctrl->shot_shift_z + (cctrl->shot_shift_z * kfx_config_state.conf.crtr_conf.exp.size_increase_on_exp * cctrl->exp_level) / 100);
    }
...
short get_creature_eye_height(const struct Thing *creatng)
{
    ...
    if (!creature_control_invalid(cctrl))
    {
        ...
        return (base_height + (base_height * kfx_config_state.conf.crtr_conf.exp.size_increase_on_exp * cctrl->exp_level) / 100);
    }
    return 0;
}
...
ThingIndex get_human_controlled_creature_target(struct Thing *thing, CrInstance inst_id, struct Packet *packet)
{
    ...
    if ((stl_x - range) < 0)
        stl_x_lower = 0;
    if (stl_x_upper > kfx_sim_state.map_subtiles_x)
        stl_x_upper = kfx_sim_state.map_subtiles_x;
    MapSubtlCoord stl_y = thing->mappos.y.stl.num;
    MapSubtlCoord stl_y_lower = stl_y - range;
    MapSubtlCoord stl_y_upper = stl_y + range;
    if (stl_y + range > kfx_sim_state.map_subtiles_y)
        stl_y_upper = kfx_sim_state.map_subtiles_y;
    if (stl_y_lower < 0)
        stl_y_lower = 0;
...
void set_first_creature(struct Thing *creatng)
{
    ...
    {
        if (kfx_sim_state.nodungeon_creatr_list_start > 0)
        {
            struct Thing* prevtng = thing_get(kfx_sim_state.nodungeon_creatr_list_start);
            struct CreatureControl* prevctrl = creature_control_get_from_thing(prevtng);
            prevctrl->players_prev_creature_idx = creatng->index;
            cctrl->players_next_creature_idx = kfx_sim_state.nodungeon_creatr_list_start;
            cctrl->players_prev_creature_idx = 0;
            kfx_sim_state.nodungeon_creatr_list_start = creatng->index;
        } else
        {
...
void remove_first_creature(struct Thing *creatng)
{
    ...
    if (is_neutral_thing(creatng))
    {
      ...
      if (thing_exists(sectng)) {
          ...
          kfx_sim_state.nodungeon_creatr_list_start = cctrl->players_next_creature_idx;
      }
...
{
    if (kfx_sim_state.thing_lists[TngList_Creatures].count >= CREATURES_COUNT-1)
        return false;

    return ((kfx_sim_state.thing_lists[TngList_Creatures].count - temp_creature) < kfx_config_state.conf.rules[0].gameplay.creatures_count);
}
...
struct Thing *find_players_lowest_level_creature_of_breed_and_gui_job(long crmodel, long job_idx, PlayerNumber plyr_idx, unsigned char pick_check)
{
    ...
    if ((!is_spec_digger) || (crmodel == CREATURE_ANY) || (crmodel == CREATURE_NOT_A_DIGGER))
    {
        ...
        {
            thing = get_player_list_creature_with_filter(kfx_sim_state.nodungeon_creatr_list_start, filter, &param);
        }
    }
...
ThingModel get_footstep_effect_element(struct Thing* thing)
{
    ...
    {
        // Default map texture
        texture = kfx_config_state.texture_id;
    }
    else
    {
...
long get_creature_thing_score(const struct Thing *thing)
{
    ...
    return kfx_sim_state.creature_scores[crmodel].value[exp];
}
...
void controlled_creature_pick_thing_up(struct Thing *creatng, struct Thing *picktng, PlayerNumber plyr_idx)
{
    ...
    {
        if ((picktng->owner != creatng->owner) && (picktng->owner != kfx_config_state.neutral_player_num) )
        {
...
void controlled_creature_drop_thing(struct Thing *creatng, struct Thing *droptng, PlayerNumber plyr_idx)
{
    ...
    if (!room_is_invalid(room))
    {
        if (room->owner == creatng->owner)
        {
            ...
            {
                if(kfx_config_state.conf.rules[creatng->owner].workers.drag_to_lair)
                {
                    if (thing_is_creature(droptng) && (creatng->owner == droptng->owner))
                    {
                        if (creature_is_being_unconscious(droptng))
                        {
                            ...
                            if ((kfx_config_state.conf.rules[creatng->owner].workers.drag_to_lair == 2)
...
void display_controlled_pick_up_thing_name(struct Thing *picktng, unsigned long timeout, PlayerNumber plyr_idx)
{
    ...
    else if (thing_is_special_box(picktng))
    {
        ...
        {
            if (kfx_sim_state.box_tooltip[picktng->custom_box.box_kind][0] == 0)
            {
...
TbBool thing_is_pickable_by_digger(struct Thing *picktng, struct Thing *creatng)
{
    ...
    else if (thing_is_creature(picktng))
    {
        ...
        {
            if ((kfx_config_state.conf.rules[creatng->owner].workers.drag_to_lair > 0) && (picktng->owner == creatng->owner))
            {
...
ThingModel get_random_creature_kind_with_model_flags(unsigned long model_flags)
{
    ...
    for (ThingModel crkind = 0; crkind < kfx_config_state.conf.crtr_conf.model_count; crkind++)
    {
        ...
        if (flag_is_set(kfx_config_state.conf.crtr_conf.model[crkind].model_flags, model_flags))
        {
...
ThingModel get_random_appropriate_creature_kind(ThingModel original_model)
{
    ...
    while (true)
    {
        random_model = GAME_RANDOM(kfx_config_state.conf.crtr_conf.model_count) + 1;
        // Exclude out-of-bounds model number.
        if (random_model >= kfx_config_state.conf.crtr_conf.model_count)
        {
            continue;
        }
...
TbResult script_use_spell_on_creature(PlayerNumber plyr_idx, struct Thing *thing, SpellKind spkind, CrtrExpLevel spell_level)
{
    ...
    { // Immunity is handled in 'apply_spell_effect_to_thing', but this command plays sounds, so check for it.
        ...
        if (flag_is_set(spconf->spell_flags, CSAfF_Disease))
        {
            ...
            cctrl->disease_caster_plyridx = kfx_config_state.neutral_player_num; // Does not spread.
        }
        return Lb_SUCCESS;
    }
    else
...
void script_move_creature(struct Thing* thing, TbMapLocation location, ThingModel effect_id)
{
    ...
    if (effect_id > 0)
    {
        create_effect(&thing->mappos, effect_id, kfx_config_state.neutral_player_num);
        create_effect(&pos, effect_id, kfx_config_state.neutral_player_num);
    }
    move_thing_in_map(thing, &pos);
    if (flag_is_set(thing->state_flags, TF1_FallingIntoAbyss)) {
        clear_flag(thing->state_flags, TF1_FallingIntoAbyss);
