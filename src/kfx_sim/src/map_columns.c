...
struct Column *get_column(long idx)
{
  ...
  return &kfx_sim_state.columns_data[idx];
}
...
struct Column *get_column_at(MapSubtlCoord stl_x, MapSubtlCoord stl_y)
{
  ...
  if (map_block_invalid(mapblk))
    return INVALID_COLUMN;
  return &kfx_sim_state.columns_data[get_mapblk_column_index(mapblk)];
}
...
TbBool column_invalid(const struct Column *colmn)
{
  ...
  if (colmn == INVALID_COLUMN)
    return true;
  return (colmn <= &kfx_sim_state.columns_data[0]) || (colmn > &kfx_sim_state.columns_data[COLUMNS_COUNT-1]) || (colmn == NULL);
}
...
long get_top_cube_at_pos(SubtlCodedCoords stl_num)
{
    ...
    if (top_pos > 0)
        ...
        tcube = kfx_sim_state.top_cube[col->floor_texture];
    return tcube;
}
...
long create_column(struct Column *col)
{
    ...
    result = 1;
    dst = &kfx_sim_state.columns_data[1];
...
void clear_columns(void)
{
  ...
  kfx_sim_state.unrevealed_column_idx = 0;
  for (i=0; i < 18; i++)
  {
    kfx_config_state.col_static_entries[i] = 0;
  }
}
...
void init_whole_blocks(void)
{
    ...
    kfx_sim_state.unrevealed_column_idx = i;
}
