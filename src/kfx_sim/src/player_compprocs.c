...
long computer_setup_dig_to_entrance(struct Computer2 *comp, struct ComputerProcess *cproc)
{
    ...
    if (right_time_to_choose_target_entrance(cproc, neutral_entrances, own_entrances, targplyr_entrances))
    {
        ...
        {
            room = INVALID_ROOM;
            dist = computer_finds_nearest_entrance2(comp, &pos, &room, kfx_config_state.neutral_player_num);
            if (dist > 0)
            {
...
long computer_setup_dig_to_gold(struct Computer2 *comp, struct ComputerProcess *cproc)
{
    ...
    {
        set_flag(cproc->flags, ComProc_Unkn0004);
        if (((kfx_sim_state.turn_last_checked_for_gold + GOLD_DEMAND_CHECK_INTERVAL) < get_gameturn()) || (kfx_sim_state.turn_last_checked_for_gold == 0))
        {
    ...
    while (stl_y_current < stl_y_end)
    {
        ...
        while (stl_x_current < stl_x_end)
        {
            ...
            if (dungeon->owner != slab_owner)
            {
                ...
                if (slab_owner != kfx_config_state.neutral_player_num || (!any_flag_is_set(slabst->block_flags, (SlbAtFlg_Valuable | SlbAtFlg_Digable | SlbAtFlg_Filled)) && slb->kind != SlbT_LAVA))
                {
