...
{
  if ((stl_x >= 0) && (stl_x < 3*kfx_sim_state.map_tiles_x))
    if ((stl_y >= 0) && (stl_y < 3*kfx_sim_state.map_tiles_y))
      return true;
  return false;
}
...
{
  if ((stl_x < 0) || (stl_x > kfx_sim_state.map_subtiles_x))
      return true;
...
struct Map *get_map_block_at(MapSubtlCoord stl_x, MapSubtlCoord stl_y)
{
  ...
  if ((stl_y < 0) || (stl_y > kfx_sim_state.map_subtiles_y))
      return INVALID_MAP_BLOCK;
  return &kfx_sim_state.map[get_subtile_number(stl_x,stl_y)];
}

struct Map *get_map_block_at_pos(SubtlCodedCoords stl_num)
{
  if ((stl_num < 0) || (stl_num > get_subtile_number(kfx_sim_state.map_subtiles_x,kfx_sim_state.map_subtiles_y)))
      return INVALID_MAP_BLOCK;
  return &kfx_sim_state.map[stl_num];
}
...
TbBool slabs_change_texture(MapSlabCoord slb_x, MapSlabCoord slb_y, MaxCoordFilterParam param)
{
    ...
    if (slabs_iter_will_change(orig_slab_kind, current_kind, fill_type))
    {
        ...
        kfx_config_state.slab_ext_data[slb_num] = target_slab_texture;
        kfx_config_state.slab_ext_data_initial[slb_num] = target_slab_texture;
        return true;
    }
    return false;
}
...
TbBool set_coords_with_range_check(struct Coord3d *pos, MapCoord cor_x, MapCoord cor_y, MapCoord cor_z, unsigned short flags)
{
    ...
    if (cor_x > subtile_coord(kfx_sim_state.map_subtiles_x,255)) {
        if (flags & MapCoord_ClipX) cor_x = subtile_coord(kfx_sim_state.map_subtiles_x,255);
        corrected = true;
    }
    if (cor_y > subtile_coord(kfx_sim_state.map_subtiles_y,255)) {
        if (flags & MapCoord_ClipY) cor_y = subtile_coord(kfx_sim_state.map_subtiles_y,255);
        corrected = true;
    }
...
{
    if (stl_x > kfx_sim_state.map_subtiles_x+1) stl_x = kfx_sim_state.map_subtiles_x+1;
    if (stl_y > kfx_sim_state.map_subtiles_y+1) stl_y = kfx_sim_state.map_subtiles_y+1;
...
void clear_mapwho(void)
{
    for (MapSubtlCoord y = 0; y < (kfx_sim_state.map_subtiles_y + 1); y++)
    {
        ...
        {
            struct Map* mapblk = &kfx_sim_state.map[get_subtile_number(x, y)];
            mapblk->mapwho = 0;
        }
  }
}
...
void reveal_map_area(PlayerNumber plyr_idx,MapSubtlCoord start_x,MapSubtlCoord end_x,MapSubtlCoord start_y,MapSubtlCoord end_y)
{
  ...
  end_x = min(stl_slab_ending_subtile(end_x)+1, kfx_sim_state.map_subtiles_x);
  end_y = min(stl_slab_ending_subtile(end_y)+1, kfx_sim_state.map_subtiles_y);
  ...
  config_reload_callbacks->panel_map_update(start_x,start_y,end_x,end_y);
}
...
{
    kfx_sim_state.map_subtiles_x = x * STL_PER_SLB;
    kfx_sim_state.map_subtiles_y = y * STL_PER_SLB;
    kfx_sim_state.map_tiles_x = x;
    ...
    kfx_sim_state.small_around_slab[2] = kfx_sim_state.map_tiles_x;
    kfx_sim_state.small_around_slab[3] = -1;

    kfx_sim_state.around_slab[0] = -kfx_sim_state.map_tiles_x - 1;
    kfx_sim_state.around_slab[1] = -kfx_sim_state.map_tiles_x;
    kfx_sim_state.around_slab[2] = -kfx_sim_state.map_tiles_x  + 1;
    kfx_sim_state.around_slab[3] = -1;
    kfx_sim_state.around_slab[4] = 0;
    kfx_sim_state.around_slab[5] = 1;
    kfx_sim_state.around_slab[6] = kfx_sim_state.map_tiles_x - 1;
    kfx_sim_state.around_slab[7] = kfx_sim_state.map_tiles_x;
    kfx_sim_state.around_slab[8] = kfx_sim_state.map_tiles_x + 1;

    kfx_sim_state.around_slab_eight[0] = -kfx_sim_state.map_tiles_x - 1;
    kfx_sim_state.around_slab_eight[1] = -kfx_sim_state.map_tiles_x;
    kfx_sim_state.around_slab_eight[2] = -kfx_sim_state.map_tiles_x  + 1;
    kfx_sim_state.around_slab_eight[3] = -1;
    kfx_sim_state.around_slab_eight[4] = 1;
    kfx_sim_state.around_slab_eight[5] = kfx_sim_state.map_tiles_x - 1;
    kfx_sim_state.around_slab_eight[6] = kfx_sim_state.map_tiles_x;
    kfx_sim_state.around_slab_eight[7] = kfx_sim_state.map_tiles_x + 1;

    kfx_sim_state.around_map[0] = -kfx_sim_state.map_subtiles_x - 2;
    kfx_sim_state.around_map[1] = -kfx_sim_state.map_subtiles_x - 1;
    kfx_sim_state.around_map[2] = -kfx_sim_state.map_subtiles_x;
    kfx_sim_state.around_map[3] = -1;
    kfx_sim_state.around_map[4] = 0;
    kfx_sim_state.around_map[5] = 1;
    kfx_sim_state.around_map[6] = kfx_sim_state.map_subtiles_x;
    kfx_sim_state.around_map[7] = kfx_sim_state.map_subtiles_x + 1;
    kfx_sim_state.around_map[8] = kfx_sim_state.map_subtiles_x + 2;
