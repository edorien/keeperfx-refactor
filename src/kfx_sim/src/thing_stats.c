...
TbBool things_stats_debug_dump(void)
{
    ...
    count[TCls_Object] = kfx_sim_state.thing_lists[TngList_Objects].count;
    count[TCls_Shot] = kfx_sim_state.thing_lists[TngList_Shots].count;
    count[TCls_EffectElem] = kfx_sim_state.thing_lists[TngList_EffectElems].count;
    count[TCls_DeadCreature] = kfx_sim_state.thing_lists[TngList_DeadCreatrs].count;
    count[TCls_Creature] = kfx_sim_state.thing_lists[TngList_Creatures].count;
    count[TCls_Effect] = kfx_sim_state.thing_lists[TngList_Effects].count;
    count[TCls_EffectGen] = kfx_sim_state.thing_lists[TngList_EffectGens].count;
    count[TCls_Trap] = kfx_sim_state.thing_lists[TngList_Traps].count;
...
{
    return (thing->owner == kfx_config_state.neutral_player_num);
}
...
HitPoints compute_creature_max_health(HitPoints base_health, CrtrExpLevel exp_level)
{
    ...
    int64_t compute_max_health = (int64_t)base_health + ((int64_t)kfx_config_state.conf.crtr_conf.exp.health_increase_on_exp * (int64_t)base_health * (int64_t)exp_level) / 100;
...
HitPoints compute_creature_spell_damage_over_time(HitPoints spell_damage, CrtrExpLevel caster_level, PlayerNumber caster_owner)
{
    ...
    HitPoints max_damage = spell_damage + (kfx_config_state.conf.crtr_conf.exp.spell_damage_increase_on_exp * spell_damage * caster_level) / 100;
    // Apply modifier.
...
long compute_creature_work_value_for_room_role(const struct Thing *creatng, RoomRole rrole, long efficiency)
{
    ...
    {
        i = compute_creature_work_value(crconf->manufacture_value* kfx_config_state.conf.rules[creatng->owner].rooms.work_efficiency, efficiency, cctrl->exp_level);
    }
    if ((rrole & RoRoF_CrTrainExp) != 0)
    {
        // Training speed does not grow with experience - otherwise it would be too fast.
        i = compute_creature_work_value(crconf->training_value* kfx_config_state.conf.rules[creatng->owner].rooms.train_efficiency, efficiency, 0);
    }
    if ((rrole & RoRoF_CrScavenge) != 0)
    {
        i = compute_creature_work_value(crconf->scavenge_value* kfx_config_state.conf.rules[creatng->owner].rooms.scavenge_efficiency, efficiency, cctrl->exp_level);
    }
...
GoldAmount calculate_correct_creature_pay(const struct Thing *thing)
{
    ...
    if (!is_neutral_thing(thing))
    {
        ...
        if (dungeon->tortured_creatures[thing->model] > 0)
            pay = (pay * kfx_config_state.conf.rules[dungeon->owner].gameplay.torture_payday) / 100;
    }
    return pay;
}
...
GoldAmount calculate_correct_creature_training_cost(const struct Thing *thing)
{
    ...
    if (!is_neutral_thing(thing))
    {
        ...
        if (dungeon->tortured_creatures[thing->model] > 0)
            training_cost = (training_cost * kfx_config_state.conf.rules[dungeon->owner].gameplay.torture_training_cost) / 100;
    }
...
GoldAmount calculate_correct_creature_scavenging_cost(const struct Thing *thing)
{
    ...
    if (!is_neutral_thing(thing))
    {
        ...
        if (dungeon->tortured_creatures[thing->model] > 0)
            scavenger_cost = (scavenger_cost * kfx_config_state.conf.rules[dungeon->owner].gameplay.torture_scavenging_cost) / 100;
    }
...
long calculate_damage_did_to_slab_with_single_hit(const struct Thing *diggertng, const struct SlabMap *slb)
{
    ...
    if (slabmap_owner(slb) == diggertng->owner)
        ...
        dig_damage = kfx_config_state.conf.rules[diggertng->owner].workers.default_imp_dig_damage;
    return dig_damage;
}
...
GoldAmount calculate_gold_digged_out_of_slab_with_single_hit(long damage_did_to_slab, const struct SlabMap *slb)
{
    ...
    GoldAmount gold = (damage_did_to_slab * gold_per_block) / kfx_sim_state.block_health[slabst->block_health_index];
    if (slb->health == 0)
    ...
    {
        gold = gold_per_block - (kfx_sim_state.block_health[slabst->block_health_index] / damage_did_to_slab) * gold;
