...
TbBool update_dungeon_build_roomspace_preview(PlayerNumber plyr_idx, MapSubtlCoord stl_x, MapSubtlCoord stl_y)
{
    ...
    if (is_my_player(player)) {
        sim_feedback->set_room_type_highlighted(player->chosen_room_kind);
    }
    ...
    return sim_feedback->tag_cursor_blocks_place_room(player->id_number, stl_x, stl_y, player->full_slab_cursor);
}
...
TbBool update_dungeon_sell_roomspace_preview(PlayerNumber plyr_idx, MapSubtlCoord stl_x, MapSubtlCoord stl_y)
{
    ...
    return sim_feedback->tag_cursor_blocks_sell_area(plyr_idx, stl_x, stl_y, player->full_slab_cursor);
}
...
void process_build_roomspace_inputs(PlayerNumber plyr_idx)
{
    ...
    else if (sim_feedback->is_square_roomspace_key_pressed()) // Define square room (mouse scroll-wheel changes size - default is 5x5)
    {
...
void process_highlight_roomspace_inputs(PlayerNumber plyr_idx)
{
    unsigned long par2;
    struct PlayerInfo* player = get_player(plyr_idx);
    if ( (sim_feedback->is_best_roomspace_key_pressed()) ) // Use "modern" click and drag method
    {
    ...
    else if ( (sim_feedback->is_square_roomspace_key_pressed())  ) // Use "modern" click and drag method
    {
        ...
        if (sim_feedback->is_roomspace_decsize_key_pressed())
        {
