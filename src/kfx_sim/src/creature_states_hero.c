...
long good_find_best_enemy_dungeon(struct Thing* creatng)
{
    ...
    for (PlayerNumber plyr_idx = 0; plyr_idx < PLAYERS_COUNT; plyr_idx++)
    {
        ...
        if (flag_is_set(kfx_config_state.conf.rules[creatng->owner].gameplay.classic_bugs_flags,ClscBug_AlwaysTunnelToRed))
        {
...
short good_back_at_start(struct Thing *thing)
{
    ...
    for (long n = 0; n < AROUND_MAP_LENGTH; n++)
    {
        ...
        {
            MapSubtlCoord stl_x = stl_num_decode_x(stl_num + kfx_sim_state.around_map[m]);
            MapSubtlCoord stl_y = stl_num_decode_y(stl_num + kfx_sim_state.around_map[m]);
...
short good_drops_gold(struct Thing *thing)
{
    ...
    if (thing->creature.gold_carried <= kfx_config_state.conf.rules[thing->owner].gameplay.pot_of_gold_holds)
...
short good_leave_through_exit_door(struct Thing *thing)
{
    ...
    cctrl->countdown = kfx_config_state.conf.rules[thing->owner].gameplay.hero_door_wait_time;
