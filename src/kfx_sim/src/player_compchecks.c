...
long computer_check_sacrifice_for_cheap_diggers(struct Computer2 *comp, struct ComputerCheck * check)
{
    ...
    if (kfx_config_state.conf.rules[dungeon->owner].sacrifices.cheaper_diggers_sacrifice_model == 0) {
    ...
    {
        struct Thing* creatng = find_creature_for_sacrifice(comp, kfx_config_state.conf.rules[dungeon->owner].sacrifices.cheaper_diggers_sacrifice_model);
...
struct Room *get_opponent_room(struct Computer2 *comp, PlayerNumber plyr_idx)
{
    ...
    for (int i = 0; i < kfx_config_state.conf.slab_conf.room_types_count; i++)
    {
        ...
        n = (n + 1) % kfx_config_state.conf.slab_conf.room_types_count;
    }
    return INVALID_ROOM;
}
...
static TbBool find_place_to_put_door_around_room(const struct Room *room, struct Coord3d *pos, PlayerNumber plyr_idx)
{
    ...
    for (long n = 0; n < SMALL_AROUND_SLAB_LENGTH; n++)
    {
        ...
        for (i = 4; i > 0; i--)
        {
            ...
            if (sim_feedback->tag_cursor_blocks_place_door(room->owner, slab_subtile_center(slb_x), slab_subtile_center(slb_y))) {
                break;
            }
