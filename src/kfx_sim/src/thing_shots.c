...
SubtlCodedCoords process_dig_shot_hit_wall(struct Thing *thing, long blocked_flags, HitPoints *health)
{
    ...
    *health = slb->health;
    ...
    if (!(kfx_config_state.conf.rules[diggertng->owner].gameplay.classic_bugs_flags & ClscBug_BreakNeutralWalls))
    {
    ...
    {
        if ((slabmap_owner(slb) != kfx_config_state.neutral_player_num) && (slabmap_owner(slb) != diggertng->owner))
        {
            return result;
        }
    }
        ...
        if ((blocked_flags & (SlbBloF_WalledX|SlbBloF_WalledY)) != 0)
        {
            if (shotng->model == ShM_Lizard)
            {
                if (shotng->shot_lizard.range >= THING_RANDOM(shotng, 90))
                {
                    ...
                    target_pos.y.val = shotng->shot_lizard.posint * kfx_config_state.conf.crtr_conf.sprite_size;
...
void shot_kill_creature(struct Thing *shotng, struct Thing *creatng)
{
    ...
    if (players_creatures_tolerate_each_other(shotng->owner,creatng->owner) &! (kfx_config_state.conf.rules[shotng->owner].gameplay.classic_bugs_flags & ClscBug_FriendlyFaint))
    {
...
long shot_hit_creature_at(struct Thing *shotng, struct Thing *trgtng, struct Coord3d *pos)
{
    ...
    {
        if (creature_is_being_unconscious(trgtng)  && !(kfx_config_state.conf.rules[trgtng->owner].gameplay.classic_bugs_flags & ClscBug_FaintedImmuneToBoulder)) //We're not actually hitting the unconscious units with a boulder
        {
            return 0;
        }
        else
...
struct Thing *get_thing_collided_with_at_satisfying_filter(struct Thing *shotng, struct Coord3d *pos, Thing_Collide_Func filter, HitTargetFlags hit_targets, long a5)
{
    ...
    MapSubtlCoord stl_y_max;
    {
        ...
        if (stl_x_max > kfx_sim_state.map_subtiles_x)
            stl_x_max = kfx_sim_state.map_subtiles_x;
        stl_y_max = coord_subtile(pos->y.val + radius);
        if (stl_y_max > kfx_sim_state.map_subtiles_y)
            stl_y_max = kfx_sim_state.map_subtiles_y;
    }
        ...
        switch (shotst->update_logic)
        {
            ...
            case ShUL_Lizard:
                ...
                else
                {
                    ...
                    target_pos.y.val = thing->shot_lizard.posint * kfx_config_state.conf.crtr_conf.sprite_size;
    ...
    switch (thing->class_id)
    {
        case TCls_Creature:
        {
            if (!thing_is_picked_up(thing) && !creature_is_being_unconscious(thing))
            {
                ...
                else if (kfx_config_state.conf.rules[thing->owner].magic.weight_calculate_push > 0 && creature_distance >= blow_distance && !creatureAlreadyAffected)
                {
...
long apply_wallhug_force_to_boulder(struct Thing *thing)
{
  ...
  if ( thing_in_wall_at(thing, &pos) )
  {
    ...
    if ( collide != 1 )
    {
      ...
      {
        thing->health -= kfx_config_state.conf.rules[thing->owner].gameplay.boulder_reduce_health_wall;
      }
...
int process_boulder_collision(struct Thing *boulder, struct Coord3d *pos, int direction_x, int direction_y)
{
    ...
    if (room_exists(room))
    {
        if (flag_is_set(get_room_kind_stats(room->kind)->flags, RoCFlg_BoulderDestroys)) // Collide with Guardposts
        {
            ...
            {
                boulder->health -= kfx_config_state.conf.rules[boulder->owner].gameplay.boulder_reduce_health_room; // decrease boulder health
            }
