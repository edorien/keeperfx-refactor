...
struct PlayerInfo *get_player_f(PlayerNumber plyr_idx,const char *func_name)
{
    ...
    {
        return &kfx_sim_state.players[plyr_idx];
    }
    if (plyr_idx == kfx_config_state.neutral_player_num) // Suppress error for never existing but valid neutral 'player'
    {
...
TbBool player_invalid(const struct PlayerInfo *player)
{
    if (player == INVALID_PLAYER)
        return true;
    return (player < &kfx_sim_state.players[0]);
}
...
TbBool players_are_enemies(PlayerNumber origin_plyr_idx, PlayerNumber check_plyr_idx)
{
    ...
    if ((origin_plyr_idx == kfx_config_state.neutral_player_num) || (check_plyr_idx == kfx_config_state.neutral_player_num))
        return false;
...
TbBool players_are_mutual_allies(PlayerNumber plyr1_idx, PlayerNumber plyr2_idx)
{
    ...
    if ((plyr1_idx == kfx_config_state.neutral_player_num) || (plyr2_idx == kfx_config_state.neutral_player_num))
        return false;
