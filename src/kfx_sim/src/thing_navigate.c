...
TbBool get_nearest_valid_position_for_creature_at(struct Thing *thing, struct Coord3d *pos)
{
    ...
    for (int i = 0; i < SPIRAL_STEPS_COUNT; i++)
    {
        ...
        else if ( stl_x > kfx_sim_state.map_subtiles_x )
        {
            stl_x = kfx_sim_state.map_subtiles_x;
        }
        ...
        else if ( stl_y > kfx_sim_state.map_subtiles_y )
        {
            stl_y = kfx_sim_state.map_subtiles_y;
        }
        ...
        if (destination_valid)
         {
             ...
             if (get_chessboard_distance(&thing->mappos, pos) > COORD_PER_STL*kfx_config_state.conf.rules[thing->owner].magic.min_distance_for_teleport)
             {
...
long get_next_gap_creature_can_fit_in_below_point(struct Thing *thing, struct Coord3d *pos)
{
    ...
    if (end_x > kfx_sim_state.map_subtiles_x * COORD_PER_STL - 1)
        end_x = kfx_sim_state.map_subtiles_x * COORD_PER_STL - 1;
    MapCoord end_y = pos->y.val + nav_radius;
    if (end_y > kfx_sim_state.map_subtiles_y * COORD_PER_STL - 1)
        end_y = kfx_sim_state.map_subtiles_y * COORD_PER_STL - 1;
