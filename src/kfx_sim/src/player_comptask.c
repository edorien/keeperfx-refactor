...
struct ComputerTask *get_computer_task(long idx)
{
    ...
    {
        return &kfx_sim_state.computer_task[idx];
    }
}

TbBool computer_task_invalid(const struct ComputerTask *ctask)
{
    if (ctask <= &kfx_sim_state.computer_task[0])
        return true;
    return false;
}
    ...
    MapSubtlCoord stl_x, MapSubtlCoord stl_y, unsigned short param1, unsigned short param2)
{
    ...
    case GA_PlaceTrap:
        ...
        return Lb_SUCCESS;
    case GA_PlaceDoor: {
        k = sim_feedback->tag_cursor_blocks_place_door(plyr_idx, stl_x, stl_y);
...
static struct ComputerTask *get_free_task(struct Computer2 *comp, TbBool use_comp_task)
{
    ...
    {
        if (++task_result >= (struct ComputerTask *)&kfx_sim_state.computer)
            return 0;
    }
    memset(task_result, 0, sizeof(struct ComputerTask));
    current_task = &kfx_sim_state.computer_task[comp->task_idx];
    if (current_task > kfx_sim_state.computer_task)
    {
        if (!use_comp_task)
        {
            if (current_task->next_task)
            {
                do
                {
                    ...
                    current_task = &kfx_sim_state.computer_task[current_task->next_task];
                } while (kfx_sim_state.computer_task[next_task].next_task);
            }
            current_task->next_task = task_result - kfx_sim_state.computer_task;
    ...
    comp->task_idx = task_result - kfx_sim_state.computer_task;
...
static TbBool other_build_here(struct Computer2 *comp, MapSubtlCoord stl_x, MapSubtlCoord stl_y, MapSlabDelta width_slabs, MapSlabDelta height_slabs)
{
    ...
    if ( task <= &kfx_sim_state.computer_task[0] )
        return true;
    while ( 1 )
    {
    ...
    MapSubtlCoord *nextstl_x, MapSubtlCoord *nextstl_y, const char *func_name)
{
    ...
    for (dig_result = TDR_DigSlab; ; dig_result++)
    {
        ...
        if (dig_result > kfx_sim_state.map_tiles_x+kfx_sim_state.map_tiles_y)
        {
...
static struct Thing *find_creature_for_call_to_arms(struct Computer2 *comp, TbBool prefer_high_scoring)
{
        ...
        i = thing_get(creature_control_get_from_thing(i)->players_next_creature_idx))
    {
        ...
        {
            if ( kfx_sim_state.creature_scores[i->model].value[cctrl->exp_level] < highest_score && !thing_is_invalid(thing) )
                continue;
            highest_score = kfx_sim_state.creature_scores[i->model].value[cctrl->exp_level];
        }
        thing = i;
    }
    return thing;
            ...
            case TDSC_DoorPlaced:
                ...
                }
                {
                    ...
                    if ((!thing_is_invalid(doortng)) && (doorst->unsellable == 0)) {
                        ...
                        value = compute_value_percentage(doorst->selling_value, kfx_config_state.conf.rules[doortng->owner].gameplay.door_sale_percent);
                        dungeon->doors_sold++;
                        dungeon->manufacture_gold += value;
...
TbBool create_task_move_creature_to_pos(struct Computer2 *comp, const struct Thing *thing, const struct Coord3d pos, CrtrStateId dst_state)
{
    ...
    if (flag_is_set(kfx_sim_state.computer_chat_flags, CChat_TasksFrequent)) {
        ...
        case CrSt_CreatureSacrifice:
            if (thing->model == kfx_config_state.conf.rules[comp->dungeon->owner].sacrifices.cheaper_diggers_sacrifice_model) {
...
TbBool create_task_sell_traps_and_doors(struct Computer2 *comp, long num_to_sell, GoldAmount gold_up_to, TbBool allow_deployed)
{
    ...
    if (flag_is_set(kfx_sim_state.computer_chat_flags, CChat_TasksScarce)) {
        sim_feedback->message_add_fmt(MsgType_Player, dungeon->owner, "I will sell some traps and doors.");
    }
