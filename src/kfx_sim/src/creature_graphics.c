...
struct PickedUpOffset *get_creature_picked_up_offset(struct Thing *thing)
{
    ...
    if ((crmodel < 1) || (crmodel >= kfx_config_state.conf.crtr_conf.model_count))
        crmodel = 0;
...
unsigned char keepersprite_frames(unsigned short n)
{
    ...
    {
        return creature_table_add[n - SIM_KEEPERSPRITE_ADD_OFFSET].FramesCount;
    }
...
unsigned char keepersprite_rotable(unsigned short n)
{
    if (n >= SIM_KEEPERSPRITE_ADD_OFFSET && n < SIM_KEEPERSPRITE_ADD_OFFSET + SIM_KEEPERSPRITE_ADD_NUM)
    {
        return creature_table_add[n - SIM_KEEPERSPRITE_ADD_OFFSET].Rotable;
    }
    if (n < CREATURE_FRAMELIST_LENGTH)
    {
        const unsigned short i = creature_list[n];
...
struct KeeperSprite * keepersprite_array(unsigned short n)
{
    if (n >= SIM_KEEPERSPRITE_ADD_OFFSET && n < SIM_KEEPERSPRITE_ADD_OFFSET + SIM_KEEPERSPRITE_ADD_NUM)
    {
        return &creature_table_add[n - SIM_KEEPERSPRITE_ADD_OFFSET];
    }
    if (n < CREATURE_FRAMELIST_LENGTH)
    {
        const unsigned short i = creature_list[n];
...
static struct KeeperSprite* sprite_by_frame(long kspr_frame)
{
    ...
    {
        return &creature_table_add[kspr_frame - SIM_KEEPERSPRITE_ADD_OFFSET];
    }
...
short get_creature_model_graphics(long crmodel, unsigned short seq_idx)
{
    ...
    if (kfx_config_state.conf.crtr_conf.creature_graphics[crmodel][seq_idx] < 0)
    {
        if ((seq_idx >= CGI_CastSpell) && (kfx_config_state.conf.crtr_conf.creature_graphics[crmodel][CGI_Attack] > 0))
        {
            return kfx_config_state.conf.crtr_conf.creature_graphics[crmodel][CGI_Attack];
        }
        return 0;
    }
