...
TbBool creature_will_generate_for_dungeon(const struct Dungeon * dungeon, ThingModel crmodel)
{
    ...
    if (kfx_sim_state.pool.crtr_kind[crmodel] <= 0) {
...
{
    if (kfx_sim_state.pool.crtr_kind[crmodel] > INT32_MIN)
    {
        kfx_sim_state.pool.crtr_kind[crmodel]--;
        return true;
    }
...
TbBool update_creature_pool_state(void)
{
    ...
    {
        if (kfx_sim_state.pool.crtr_kind[i] > 0)
        { kfx_sim_state.pool.is_empty = false; break; }
    }
    return true;
}
...
void add_creature_to_pool(ThingModel kind, int32_t amount)
{
    ...
    if (amount > 0 && kfx_sim_state.pool.crtr_kind[kind] > INT32_MAX - amount)
    {
        kfx_sim_state.pool.crtr_kind[kind] = INT32_MAX;
    }
    else if (amount < 0 && kfx_sim_state.pool.crtr_kind[kind] < INT32_MIN - amount)
    {
        kfx_sim_state.pool.crtr_kind[kind] = INT32_MIN;
    }
    else
    {
        kfx_sim_state.pool.crtr_kind[kind] += amount;
    }
}
