...
{
    memset(dungeon->creatures_scavenging, 0, kfx_config_state.conf.crtr_conf.model_count);
...
TbBool can_scavenge_creature_from_pool(const struct Dungeon * dungeon, ThingModel crtr_kind)
{
    ...
    if (kfx_sim_state.pool.crtr_kind[crtr_kind] <= 0) {
        return false;
    }
...
TbBool thing_is_valid_scavenge_target(const struct Thing *calltng, const struct Thing *scavtng)
{
    ...
    if (is_hero_thing(scavtng) && (!kfx_config_state.conf.rules[calltng->owner].rooms.scavenge_good_allowed)) {
        return false;
    }
    if (is_neutral_thing(scavtng) && (!kfx_config_state.conf.rules[calltng->owner].rooms.scavenge_neutral_allowed)) {
        return false;
    }
...
CrCheckRet process_scavenge_function(struct Thing *calltng)
{
    ...
    if (callctrl->turns_at_job > kfx_config_state.conf.rules[calltng->owner].rooms.scavenge_cost_frequency)
    {
