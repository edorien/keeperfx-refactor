...
{
    if (kfx_sim_state.thing_lists[TngList_EffectElems].index > 0) {
        struct Thing *old_effect = thing_get(kfx_sim_state.thing_lists[TngList_EffectElems].index);
...
static struct Thing *allocate_thing(enum ThingAllocationPool pool_type, const char *func_name)
{
    ...
    if (pool_type == ThingAllocation_Unsynced) {
        free_list = kfx_sim_state.unsynced_free_things;
        count = &kfx_sim_state.unsynced_free_things_count;
        list_name = "unsynced";
    } else {
        free_list = kfx_sim_state.synced_free_things;
        count = &kfx_sim_state.synced_free_things_count;
        list_name = "synced";
    }
...
struct Thing *allocate_free_thing_structure_f(unsigned char class_id, const char *func_name)
{
    if (is_non_synchronized_thing_class(class_id)) {
        if (kfx_sim_state.unsynced_free_things_count == 0) {
...
TbBool i_can_allocate_free_thing_structure(unsigned char class_id)
{
    ...
    if (kfx_sim_state.synced_free_things_count > 0) {
        return true;
    }
...
void delete_thing_structure_f(struct Thing *thing, TbBool deleting_everything, const char *func_name)
{
    ...
    if (thing->index > 0) {
        if (thing->index <= SYNCED_THINGS_COUNT) {
            push_free_thing_index(kfx_sim_state.synced_free_things, &kfx_sim_state.synced_free_things_count, SYNCED_THINGS_COUNT, thing->index);
        } else {
...
short thing_is_invalid(const struct Thing *thing)
{
    if (thing == NULL)
        return true;
    return (thing <= &kfx_sim_state.things_data[0]) || (thing > &kfx_sim_state.things_data[THINGS_COUNT-1]);
}
