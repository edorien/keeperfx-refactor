 ...
 * @par Purpose:
 *     Defines elements of kfx_config_state.conf.crtr_conf.states[] array, containing valid creature states.
 * @par Comment:
 *     None.
 * @author   Tomasz Lis
...
{
  if (stati <= &kfx_config_state.conf.crtr_conf.states[0])
    return true;
  return false;
}
...
TbBool creature_can_be_set_unconscious(const struct Thing *creatng, const struct Thing *killertng, CrDeathFlags flags)
{
    ...
    if (THING_RANDOM(creatng, 100) >= kfx_config_state.conf.rules[creatng->owner].creature.stun_without_prison_chance)
    {
    ...
    if (flag_is_set(get_creature_model_flags(creatng), CMF_IsEvil) && (THING_RANDOM(creatng, 100) >= kfx_config_state.conf.rules[creatng->owner].creature.stun_enemy_chance_evil))
    {
        return false;
    }
    if (!flag_is_set(get_creature_model_flags(creatng), CMF_IsEvil) && (THING_RANDOM(creatng, 100) >= kfx_config_state.conf.rules[creatng->owner].creature.stun_enemy_chance_good))
    {
        return false;
    }
...
{
    CrtrStateId i = get_creature_state_besides_interruptions(thing);
    if (kfx_config_state.conf.crtr_conf.states[i].state_type == CrStTyp_AngerJob)
        return true;
    return false;
}
...
{
    CrtrStateId crstate = get_creature_state_besides_move(thing);
    if (kfx_config_state.conf.crtr_conf.states[crstate].state_type == 0)
        return true;
    return false;
}
...
/**
 * Fills given array with information about kfx_sim_state.small_around_slab[] elements which are accessible to move to.
...
SubtlCodedCoords find_position_around_in_room(const struct Coord3d *pos, PlayerNumber owner, RoomKind rkind, struct Thing* thing)
{
    ...
    for (long n = 0; n < AROUND_MAP_LENGTH; n++)
    {
        ...
        if (kfx_sim_state.around_map[m] == 0)
        {
...
short creature_being_dropped(struct Thing *creatng)
{
    ...
    if (!creature_under_spell_effect(creatng, CSAfF_Chicken))
    {
        ...
        {
            if ((slabmap_owner(slb) == creatng->owner) || (slabmap_owner(slb) == kfx_config_state.neutral_player_num))
            {
...
void set_creature_size_stuff(struct Thing *creatng)
{
    ...
    if (creature_under_spell_effect(creatng, CSAfF_Chicken))
    {
        creatng->sprite_size = kfx_config_state.conf.crtr_conf.sprite_size;
    }
    else
    {
        creatng->sprite_size = kfx_config_state.conf.crtr_conf.sprite_size + (kfx_config_state.conf.crtr_conf.sprite_size * kfx_config_state.conf.crtr_conf.exp.size_increase_on_exp * cctrl->exp_level) / 100;
    }
}
...
short creature_change_to_chicken(struct Thing *creatng)
{
    ...
    if (cctrl->countdown > 0)
    {
        ...
        if (!thing_is_invalid(efftng))
        {
            set_thing_draw(efftng, 819, 0, 1200 * cctrl->countdown / 10 + kfx_config_state.conf.crtr_conf.sprite_size, -1, 0, ODC_Default);
            clear_flag(efftng->rendering_flags, TRF_Transpar_Flags);
            set_flag(efftng->rendering_flags, TRF_Transpar_8);
        }
        return 0;
...
{
    struct CreatureStateConfig* active_stati = &kfx_config_state.conf.crtr_conf.states[active_state];
    struct CreatureStateConfig* continue_stati = &kfx_config_state.conf.crtr_conf.states[continue_state];
...
short creature_take_salary(struct Thing *creatng)
{
    ...
    if (!(kfx_config_state.conf.rules[creatng->owner].gameplay.classic_bugs_flags & ClscBug_FullyHappyWithGold))
    {
...
void make_creature_unconscious(struct Thing *creatng)
{
    ...
    set_creature_size_stuff(creatng);
    struct CreatureControl* cctrl = creature_control_get_from_thing(creatng);
    if (kfx_config_state.conf.rules[creatng->owner].gameplay.classic_bugs_flags & ClscBug_ResurrectRemoved)
    {
    ...
    cctrl->creature_control_flags |= CCFlg_NoCompControl;
    cctrl->conscious_back_turns = kfx_config_state.conf.rules[creatng->owner].creature.game_turns_unconscious;
}
...
void create_effect_around_thing(struct Thing *thing, long eff_kind)
{
    ...
    if (coord_x_end >= subtile_coord(kfx_sim_state.map_subtiles_x+1, 0) - 1)
        coord_x_end = subtile_coord(kfx_sim_state.map_subtiles_x+1, 0) - 1;
    MapCoord coord_y_beg = (MapCoord)thing->mappos.y.val - tng_radius;
    if (coord_y_beg < 0)
        coord_y_beg = 0;
    MapCoord coord_y_end = (MapCoord)thing->mappos.y.val + tng_radius;
    if (coord_y_end >= subtile_coord(kfx_sim_state.map_subtiles_y+1, 0) - 1)
        coord_y_end = subtile_coord(kfx_sim_state.map_subtiles_y+1, 0) - 1;
...
TbBool process_creature_hunger(struct Thing *thing)
{
    ...
    if (kfx_config_state.conf.rules[thing->owner].health.turns_per_hunger_health_loss > 0)
    {
...
short state_cleanup_dragging_object(struct Thing *creatng)
{
    ...
    if (cctrl->dragtng_idx > 0)
    {
        ...
        objctng->owner = kfx_config_state.neutral_player_num;
    }
    return 1;
}
...
long process_creature_needs_a_wage(struct Thing *creatng, const struct CreatureModelConfig *crconf)
{
    ...
    room = find_any_navigable_room_for_thing_closer_than(creatng, creatng->owner, RoRoF_GoldStorage, NavRtF_Default, kfx_sim_state.map_subtiles_x / 2 + kfx_sim_state.map_subtiles_y / 2);
    if (room_is_invalid(room))
    {
