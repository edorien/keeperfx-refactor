...
void set_chosen_power(PowerKind pwkind, TextStringId sptooltip)
{
    ...
    kfx_sim_state.chosen_spell_type = pwkind;
    kfx_sim_state.chosen_spell_spridx = powerst->bigsym_sprite_idx;
    kfx_sim_state.chosen_spell_tooltip = sptooltip;
}

void set_chosen_power_none(void)
{
    SYNCDBG(6,"Setting to %d",0);
    kfx_sim_state.chosen_spell_type = 0;
    kfx_sim_state.chosen_spell_spridx = 0;
    kfx_sim_state.chosen_spell_tooltip = 0;
}
...
void process_armageddon(void)
{
    ...
    long i;
    SYNCDBG(6,"Starting");
    GameTurnDelta countdown = kfx_config_state.conf.rules[kfx_sim_state.armageddon_caster_idx].magic.armageddon_count_down;
    if (kfx_sim_state.armageddon_cast_turn == 0)
        return;
    if ((kfx_sim_state.armageddon_cast_turn + countdown) > get_gameturn())
    {
        if (player_cannot_win(kfx_sim_state.armageddon_caster_idx))
        {
...
void teleport_armageddon_influenced_creature(struct Thing* creatng)
{
    ...
    move_thing_in_map(creatng, &kfx_sim_state.armageddon_mappos);
...
TbBool players_disease_can_infect_target_players_creatures(PlayerNumber source_player, PlayerNumber target_player)
{
    if (source_player == target_player)
        return false;
    if (source_player == kfx_config_state.neutral_player_num)
        return false;
    if (players_are_enemies(source_player, target_player))
        return true;

    struct PlayerInfo* player = get_player(source_player);
    if (kfx_config_state.conf.rules[source_player].gameplay.allies_share_disease)
...
void process_disease(struct Thing *creatng)
{
    ...
    if (THING_RANDOM(creatng, 100) < kfx_config_state.conf.rules[creatng->owner].magic.disease_transfer_percentage)
    {
    ...
    if (((get_gameturn() - cctrl->disease_start_turn) % kfx_config_state.conf.rules[creatng->owner].magic.disease_lose_health_time) == 0)
    {
        apply_damage_to_thing_and_display_health(creatng, kfx_config_state.conf.rules[creatng->owner].magic.disease_lose_percentage_health * cctrl->max_health / 100, cctrl->disease_caster_plyridx);
    }
}
        ...
        if ( (stl_y >= 0) && (stl_y <= kfx_sim_state.map_subtiles_y) )
        {
            ...
            for (long soe_x = 0; soe_x <= MAX_SOE_RADIUS; soe_x++, stl_x++)
            {
                if (dungeon->soe_explored_flags[soe_y][soe_x])
                {
                    ...
                    if (boundstl_x > kfx_sim_state.map_subtiles_x-1)
                    {
                        boundstl_x = kfx_sim_state.map_subtiles_x-1;
                    }
                    if (boundstl_x >= stl_x)
                    {
        ...
        if ( (stl_x >= 0) && (stl_x <= 255) )
        {
            ...
            for (long soe_y = 0; soe_y <= MAX_SOE_RADIUS; soe_y++, stl_y++)
            {
                if (dungeon->soe_explored_flags[soe_y][soe_x])
                {
                    ...
                    if (boundstl_y > kfx_sim_state.map_subtiles_y-1)
                    {
                        boundstl_y = kfx_sim_state.map_subtiles_y-1;
                    }
                    if (stl_y < 0)
                    {
                        stl_y = 0;
...
void process_timebomb(struct Thing *creatng)
{
    ...
    {
        if ((cctrl->timebomb_countdown % kfx_sim_state.turns_per_second) == 0)
        {
            long time = (cctrl->timebomb_countdown / kfx_sim_state.turns_per_second);
