...
{
  if (slb_x > kfx_sim_state.map_tiles_x) slb_x = kfx_sim_state.map_tiles_x;
  if (slb_y > kfx_sim_state.map_tiles_y) slb_y = kfx_sim_state.map_tiles_y;
  if (slb_x < 0)  slb_x = 0;
  if (slb_y < 0) slb_y = 0;
  return slb_y*(kfx_sim_state.map_tiles_x) + (SlabCodedCoords)slb_x;
}
...
{
  return slb_num % (kfx_sim_state.map_tiles_x);
}
...
{
  return (slb_num/(kfx_sim_state.map_tiles_x))%kfx_sim_state.map_tiles_y;
}
...
struct SlabMap *get_slabmap_direct(SlabCodedCoords slab_num)
{
  ...
  return &kfx_sim_state.slabmap[slab_num];
}
...
{
  if ((slab_x < 0) || (slab_x >= kfx_sim_state.map_tiles_x))
      return INVALID_SLABMAP_BLOCK;
  if ((slab_y < 0) || (slab_y >= kfx_sim_state.map_tiles_y))
      return INVALID_SLABMAP_BLOCK;
  return &kfx_sim_state.slabmap[slab_y*(kfx_sim_state.map_tiles_x) + slab_x];
}
...
struct SlabMap *get_slabmap_for_subtile(MapSubtlCoord stl_x, MapSubtlCoord stl_y)
{
    ...
    if ((stl_y < 0) || (stl_y >= kfx_sim_state.map_subtiles_y))
        return INVALID_SLABMAP_BLOCK;
    return &kfx_sim_state.slabmap[subtile_slab(stl_y)*(kfx_sim_state.map_tiles_x) + subtile_slab(stl_x)];
}
...
{
    if (thing_is_invalid(thing))
        return kfx_config_state.neutral_player_num;
...
TbBool slabmap_block_invalid(const struct SlabMap *slb)
{
    ...
    if (slb == INVALID_SLABMAP_BLOCK)
        return true;
    return (slb < &kfx_sim_state.slabmap[0]);
}
...
{
  if ((slb_x < 0) || (slb_x >= kfx_sim_state.map_tiles_x))
      return true;
  if ((slb_y < 0) || (slb_y >= kfx_sim_state.map_tiles_y))
      return true;
  return false;
}
...
/**
 * Returns slab kind of given (already-validated) SlabMap. Bare-extern'd
 * by config_terrain.c's get_slab_stats() so it doesn't need struct
 * SlabMap visible by value. See docs/refactor/
...
void set_slab_owner(MapSlabCoord slb_x, MapSlabCoord slb_y, PlayerNumber owner)
{
    ...
    {
        kfx_config_state.slab_ext_data[get_slab_number(slb_x,slb_y)] = kfx_config_state.slab_ext_data_initial[get_slab_number(slb_x,slb_y)];
    }
    else
    {
        kfx_config_state.slab_ext_data[get_slab_number(slb_x,slb_y)] = dungeon->texture_pack;
    }
    slb->owner = owner;
    if(old_owner != owner)
    {
...
void update_blocks_around_slab(MapSlabCoord slb_x, MapSlabCoord slb_y)
{
    ...
    if (ey > kfx_sim_state.map_subtiles_y)
        ey = kfx_sim_state.map_subtiles_y;
    MapSubtlCoord ex = stl_x + 5;
    if (ex > kfx_sim_state.map_subtiles_x)
        ex = kfx_sim_state.map_subtiles_x;
            ...
            for (int k = -1; k < 2; k+=2)
            {
                ...
                if ( (slabst2->category == SlbAtCtg_FortifiedWall) || (slabst2->category == SlbAtCtg_FriableDirt) )
                {
                    ...
                    if ( (slabst3->category == SlbAtCtg_FortifiedWall) && (slabmap_owner(slb3) != keep_plyr_idx) )
                    {
                        ...
                        {
                            SlabKind slbkind = alter_rock_style(SlbT_EARTH, x3, y3, kfx_config_state.neutral_player_num);
                            place_slab_type_on_map(slbkind, slab_subtile_center(x3), slab_subtile_center(y3), kfx_config_state.neutral_player_num, 0);
    ...
    for (long n = 0; n < SMALL_AROUND_SLAB_LENGTH; n++)
    {
        ...
        if ((slabst->category == SlbAtCtg_FortifiedWall) && (slabmap_owner(slb) != keep_plyr_idx))
        {
            if (!slab_by_players_land(slabmap_owner(slb), sslb_x, sslb_y))
            {
                ...
                place_slab_type_on_map(newslab, slab_subtile_center(sslb_x), slab_subtile_center(sslb_y), kfx_config_state.neutral_player_num, 0);
    ...
    for (MapSlabCoord slb_y=0; slb_y < kfx_sim_state.map_tiles_y; slb_y++)
    {
        for (MapSlabCoord slb_x=0; slb_x < kfx_sim_state.map_tiles_x; slb_x++)
        {
            ...
            if (slabmap_owner(slb) == plyr_idx)
            {
                ...
                {
                    kfx_config_state.slab_ext_data[get_slab_number(slb_x,slb_y)] = texture_id;
                }
            }
        }
    }
