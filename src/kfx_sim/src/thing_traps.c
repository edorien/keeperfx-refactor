...
TbBool update_trap_trigger_line_of_sight_90(struct Thing *traptng)
{
    ...
    {
        line_of_sight_90_range = max(kfx_sim_state.map_subtiles_x, kfx_sim_state.map_subtiles_y);
    }
    ...
    MapSubtlCoord stl_y_aft;
    {
        ...
        if (stl_y_aft >= kfx_sim_state.map_subtiles_y+1)
            stl_y_aft = kfx_sim_state.map_subtiles_y+1;
        stl_x_pre = stl_x_beg - line_of_sight_90_range;
        if (stl_x_pre <= 0)
            stl_x_pre = 0;
        stl_x_aft = stl_x_end + line_of_sight_90_range;
        if (stl_x_aft >= kfx_sim_state.map_subtiles_x+1)
            stl_x_aft = kfx_sim_state.map_subtiles_x+1;
    }
    MapSubtlCoord stl_x;
    MapSubtlCoord stl_y;
...
void activate_trap_slab_change(struct Thing *traptng)
{
    ...
    place_slab_type_on_map(slab, stl_x, stl_y, kfx_config_state.neutral_player_num, 0);
    // TODO
