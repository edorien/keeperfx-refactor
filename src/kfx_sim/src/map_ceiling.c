...
static int ceiling_calculate_height_from_nearest_walls(int result, int number_of_steps)
{
    ...
    if (result >= kfx_sim_state.ceiling_height_max)
    {
        if (result > kfx_sim_state.ceiling_height_max)
        {
            ...
            if (result <= kfx_sim_state.ceiling_height_min)
                return kfx_sim_state.ceiling_height_min;
        }
    }
    else
    {
        result += step_adjustment;
        if (result >= kfx_sim_state.ceiling_height_max)
            return kfx_sim_state.ceiling_height_max;
    }
    return result;
}
...
void ceiling_partially_recompute_heights(MapSubtlCoord sx, MapSubtlCoord sy, MapSubtlCoord ex, MapSubtlCoord ey)
{
    ...
    if (ex + ceil_dist >= (kfx_sim_state.map_subtiles_x + 1))
        computation_end_stl_x = (kfx_sim_state.map_subtiles_x + 1);
    MapSubtlCoord computation_end_stl_y = ey + ceil_dist;
    if (computation_end_stl_y >= (kfx_sim_state.map_subtiles_y + 1))
        computation_end_stl_y = (kfx_sim_state.map_subtiles_y + 1);

    //ceiling_cache = (signed char*)scratch;

    MapSubtlCoord solid_check_start_stl_x = computation_start_stl_x - kfx_sim_state.ceiling_dist;
    if (solid_check_start_stl_x <= 0)
        solid_check_start_stl_x = 0;

    MapSubtlCoord solid_check_start_stl_y = computation_start_stl_y - kfx_sim_state.ceiling_dist;
    if (solid_check_start_stl_y <= 0)
        solid_check_start_stl_y = 0;

    MapSubtlCoord solid_check_end_stl_x = computation_end_stl_x + kfx_sim_state.ceiling_dist;
    if (solid_check_end_stl_x >= (kfx_sim_state.map_subtiles_x + 1))
        solid_check_end_stl_x = (kfx_sim_state.map_subtiles_x + 1);
    MapSubtlCoord solid_check_end_stl_y = computation_end_stl_y + kfx_sim_state.ceiling_dist;
    if (solid_check_end_stl_y >= (kfx_sim_state.map_subtiles_y + 1))
        solid_check_end_stl_y = (kfx_sim_state.map_subtiles_y + 1);
    ...
    while (current_stl_y < computation_end_stl_y)
    {
        ...
        while (computation_end_stl_x > current_stl_x)
        {
            ...
            if (ceiling_height <= -1)
            {
                ...
                spiral_step_count = 0;
                spir = spiral_step;
                if (kfx_sim_state.ceiling_search_dist > 0)
                {
                    while (1)
                    {
                        search_stl_x = current_stl_x + spir->h;
                        search_stl_y = current_stl_y + spir->v;
                        if (search_stl_x >= 0 && search_stl_x < kfx_sim_state.map_subtiles_x && search_stl_y >= 0 && search_stl_y < kfx_sim_state.map_subtiles_y)
                        {
                        ...
                        ++spiral_step_count;
                        ++spir;
                        if (spiral_step_count >= kfx_sim_state.ceiling_search_dist)
                ...
                if (near_wall)
                    ...
                    ceiling_height = kfx_sim_state.ceiling_height_max;
            }
...
void ceiling_init()
{
    for (MapSubtlCoord stl_y=0; stl_y < kfx_sim_state.map_subtiles_y; stl_y++)
    {
        for (MapSubtlCoord stl_x=0; stl_x < kfx_sim_state.map_subtiles_x; stl_x++)
        {
            ...
            if (filled_h <= -1)
            {
              if (kfx_sim_state.ceiling_search_dist <= 0)
              {
                  filled_h = kfx_sim_state.ceiling_height_max;
              }
              else
              {
                int i = 0;
                while ( 1 )
                {
                    ...
                    if ((cstl_x >= 0) && (cstl_x <= kfx_sim_state.map_subtiles_x))
                    {
                        if ((cstl_y >= 0) && (cstl_y <= kfx_sim_state.map_subtiles_y))
                        {
                            ...
                            if (filled_h > -1)
                            {
                                ...
                                {
                                    filled_h += kfx_sim_state.ceiling_step * delta_max;
                                    if (filled_h > kfx_sim_state.ceiling_height_max)
                                        filled_h = kfx_sim_state.ceiling_height_max;
                                } else
                                if ( filled_h > kfx_sim_state.ceiling_height_max )
                                {
                                    filled_h -= kfx_sim_state.ceiling_step * delta_max;
                                    if (filled_h < kfx_sim_state.ceiling_height_min)
                                        filled_h = kfx_sim_state.ceiling_height_min;
                                }
                                break;
                            }
                        }
                    ...
                    i++;
                    if (i >= kfx_sim_state.ceiling_search_dist) {
                        filled_h = kfx_sim_state.ceiling_height_max;
                        break;
                    }
                }
...
short ceiling_set_info(long height_max, long height_min, long step)
{
    ...
    if ( dist >= 2500 )
      dist = 2500;
    kfx_sim_state.ceiling_dist = dist;
    if (dist > 20)
    {
      ERRORLOG("Ceiling search distance too big");
      return 0;
    }
    kfx_sim_state.ceiling_height_max = height_max;
    kfx_sim_state.ceiling_height_min = height_min;
    kfx_sim_state.ceiling_step = step;
    kfx_sim_state.ceiling_search_dist = (2*kfx_sim_state.ceiling_dist+1) * (2*kfx_sim_state.ceiling_dist+1);
    return 1;
}
