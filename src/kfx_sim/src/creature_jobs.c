...
TbBool creature_find_and_perform_anger_job(struct Thing *creatng)
{
    ...
    for (k = 0; k < kfx_config_state.conf.crtr_conf.angerjobs_count; k++)
    {
        ...
        i = (i+1) % kfx_config_state.conf.crtr_conf.angerjobs_count;
    }
    return 0;
}
...
{
    if (player_uses_power_obey(creatng->owner) && ((kfx_config_state.conf.rules[creatng->owner].gameplay.classic_bugs_flags & ClscBug_MustObeyKeepsNotDoJobs) == 0)) {
        return false;
    }
    struct CreatureModelConfig* crconf = creature_stats_get_from_thing(creatng);
