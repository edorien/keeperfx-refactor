...
struct CreatureBattle *creature_battle_get(BattleIndex battle_id)
{
    ...
    return &kfx_sim_state.battles[battle_id];
}
...
struct CreatureBattle *creature_battle_get_from_thing(const struct Thing *thing)
{
    ...
    return &kfx_sim_state.battles[cctrl->battle_id];
}
...
{
  return (battle <= &kfx_sim_state.battles[0]) || (battle == NULL);
}
...
long battle_move_player_towards_battle(struct PlayerInfo *player, BattleIndex battle_idx)
{
    ...
    if (!thing_exists(thing))
    {
        ...
        player->zoom_to_pos_x = subtile_coord_center(kfx_sim_state.map_subtiles_x/2);
        player->zoom_to_pos_y = subtile_coord_center(kfx_sim_state.map_subtiles_y/2);
        return 0;
    }
