...
short creature_combat_flee(struct Thing *creatng)
{
    ...
    if (get_chessboard_distance(&creatng->mappos, &cctrl->flee_pos) >= slab_coord(2))
    {
        ...
        if ((turns_in_flee <= kfx_config_state.conf.rules[creatng->owner].creature.game_turns_in_flee) || creature_under_spell_effect(creatng, CSAfF_Fear))
        {
            GameTurnDelta escape_turns = (kfx_config_state.conf.rules[creatng->owner].creature.game_turns_in_flee >> 2);
            if (escape_turns <= 50)
                escape_turns = 50;
...
struct Thing *get_thing_collided_with_at_satisfying_filter_in_square_of(struct Thing *creatng, struct Coord3d *pos, long square_size, Thing_Collide_Func filter, HitTargetFlags filter_par1, long filter_par2)
{
    ...
    if (stl_x_end >= kfx_sim_state.map_subtiles_x)
        stl_x_end = kfx_sim_state.map_subtiles_x;
    MapSubtlCoord stl_y_end = coord_subtile(pos->y.val + square_size / 2);
    if (stl_y_end >= kfx_sim_state.map_subtiles_y)
        stl_y_end = kfx_sim_state.map_subtiles_y;
...
TbBool creature_look_for_combat(struct Thing *creatng)
{
    ...
    if ((cctrl->opponents_melee_count == 0) && (cctrl->opponents_ranged_count == 0))
    {
        ...
        if (kfx_config_state.conf.crtr_conf.states[crstate].sneaky == 1)
            return false;
    }
