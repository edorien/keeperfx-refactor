...
struct StructureList *get_list_for_thing_class(ThingClass class_id)
{
    ...
    {
    case TCls_Object:
        return &kfx_sim_state.thing_lists[TngList_Objects];
    case TCls_Shot:
        return &kfx_sim_state.thing_lists[TngList_Shots];
    case TCls_EffectElem:
        return &kfx_sim_state.thing_lists[TngList_EffectElems];
    case TCls_DeadCreature:
        return &kfx_sim_state.thing_lists[TngList_DeadCreatrs];
    case TCls_Creature:
        return &kfx_sim_state.thing_lists[TngList_Creatures];
    case TCls_Effect:
        return &kfx_sim_state.thing_lists[TngList_Effects];
    case TCls_EffectGen:
        return &kfx_sim_state.thing_lists[TngList_EffectGens];
    case TCls_Trap:
        return &kfx_sim_state.thing_lists[TngList_Traps];
    case TCls_Door:
        return &kfx_sim_state.thing_lists[TngList_Doors];
    case TCls_AmbientSnd:
        return &kfx_sim_state.thing_lists[TngList_AmbientSnds];
    case TCls_CaveIn:
        return &kfx_sim_state.thing_lists[TngList_CaveIns];
    default:
        return NULL;
    }
}
...
void update_things(void)
{
    ...
    update_things_in_list(&kfx_sim_state.thing_lists[TngList_Shots]);
    update_things_in_list(&kfx_sim_state.thing_lists[TngList_Objects]);
    update_things_in_list(&kfx_sim_state.thing_lists[TngList_Effects]);
    update_things_in_list(&kfx_sim_state.thing_lists[TngList_EffectElems]);
    update_things_in_list(&kfx_sim_state.thing_lists[TngList_DeadCreatrs]);
    update_things_in_list(&kfx_sim_state.thing_lists[TngList_EffectGens]);
    update_things_in_list(&kfx_sim_state.thing_lists[TngList_Doors]);
    update_things_sounds_in_list(&kfx_sim_state.thing_lists[TngList_AmbientSnds]);
...
void init_player_start(struct PlayerInfo *player, TbBool keep_prev)
{
    ...
    } else
    {
        ...
        if (!keep_prev)
        {
            dungeon->mappos.x.val = subtile_coord_center(kfx_sim_state.map_subtiles_x/2);
            dungeon->mappos.y.val = subtile_coord_center(kfx_sim_state.map_subtiles_y/2);
    ...
    MapCoord max_dist, HitPoints max_damage, PlayerNumber owner)
{
    ...
    if ((tngdst->class_id == TCls_Creature) && (tngdst->owner == owner)) {
        max_dist = max_dist * kfx_config_state.conf.rules[tngsrc->owner].magic.friendly_fight_area_range_percent / 100;
        max_damage = max_damage * kfx_config_state.conf.rules[tngsrc->owner].magic.friendly_fight_area_damage_percent / 100;
    }
...
struct Thing *get_player_list_nth_creature_of_model_on_territory(long thing_idx, ThingModel crmodel, long crtr_idx, int friendly)
{
    ...
    while (i != 0)
    {
        ...
        {
            if (slbwnr == kfx_config_state.neutral_player_num)
            {
                match = 1;
            }
        }
...
TbBool gold_pile_with_maximum_at_xy(MapSubtlCoord stl_x, MapSubtlCoord stl_y)
{
    ...
    while (i != 0)
    {
        ...
        if ((thing->class_id == TCls_Object) && (object_is_gold_laying_on_ground(thing)))
        {
            if (thing->valuable.gold_stored >= kfx_config_state.conf.rules[thing->owner].gameplay.gold_pile_maximum)
            {
                return true;
            }
        }
...
struct Thing *get_nearest_thing_at_position(MapSubtlCoord stl_x, MapSubtlCoord stl_y)
{
  ...
  do
  {
    n = 0;
    y = stl_y + k;
    if ( (y >= 0) && (y < kfx_sim_state.map_subtiles_y) )
    {
      do
      {
        x = stl_x + n;
        if ( (x >= 0) && (x < kfx_sim_state.map_subtiles_x) )
        {
...
long switch_owned_objects_on_destoyed_slab_to_neutral(MapSlabCoord slb_x, MapSlabCoord slb_y, PlayerNumber prev_owner)
{
    ...
    param.class_id = 0;
    param.model_id = 0;
    param.primary_number = kfx_config_state.neutral_player_num;
