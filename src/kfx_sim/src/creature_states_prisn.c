...
TbBool jailbreak_possible(struct Room *room, PlayerNumber creature_owner)
{
    ...
    if (creature_owner == kfx_config_state.neutral_player_num || room->owner == creature_owner)
    {
        return false;
    }
    unsigned long k = 0;
...
TbBool process_prisoner_skelification(struct Thing *thing, struct Room *room)
{
    ...
    if (THING_RANDOM(thing, 101) > kfx_config_state.conf.rules[room->owner].rooms.prison_skeleton_chance) {
        return false;
    }
...
CrCheckRet process_prison_function(struct Thing *creatng)
{
    ...
    else if ((creatng->health < 0) && ((!crconf->humanoid_creature) && ((crconf->prison_kind > kfx_config_state.conf.crtr_conf.model_count) || (crconf->prison_kind <= 0))))
    {
        if (is_my_player_number(room->owner))
        {
    ...
    if (((get_gameturn() % kfx_config_state.conf.rules[room->owner].rooms.time_between_prison_break) == 0) &&
        (get_gameturn() > cctrl->imprison.start_gameturn + kfx_config_state.conf.rules[room->owner].rooms.time_in_prison_without_break))
    {
