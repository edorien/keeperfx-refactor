...
TbBool creature_can_be_trained(const struct Thing *thing)
{
    ...
    if (crconf->training_value <= 0)
        return false;
    if ((cctrl->exp_level >= kfx_config_state.conf.rules[thing->owner].rooms.training_room_max_level-1) &! (kfx_config_state.conf.rules[thing->owner].rooms.training_room_max_level == 0))
        return false;
...
CrStateRet training(struct Thing *thing)
{
    ...
    cctrl->turns_at_job++;
    if (cctrl->turns_at_job >= kfx_config_state.conf.rules[thing->owner].rooms.train_cost_frequency)
    {
