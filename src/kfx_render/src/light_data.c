...
TbBool light_is_invalid(const struct Light *lgt)
{
    if (lgt == NULL)
        return true;
    if ((lgt < &lish.lights[1]) || (lgt > &lish.lights[LIGHTS_COUNT-1]))
        return true;
    return false;
}
...
TbBool light_shadow_cache_invalid(struct ShadowCache *shdc)
{
    if (shdc == NULL)
        return true;
    if ((shdc < &lish.shadow_cache[1]) || (shdc > &lish.shadow_cache[SHADOW_CACHE_COUNT-1]))
        return true;
    return false;
}
...
void light_remove_light_from_list(struct Light *lgt, struct StructureList *list)
{
  ...
  if ( flag_is_set(lgt->flags2,LgtF2_InList) )
  {
    ...
    {
      lgt2 = &lish.lights[list->index];
      for ( i = 0; lgt2 != lish.lights; lgt2 = &lish.lights[lgt2->next_in_list] )
      {
        if ( lgt2 == lgt )
        {
          Removed = true;
...
void light_signal_stat_light_update_in_area(long x1, long y1, long x2, long y2)
{
  ...
  while ( lgt < (struct Light *)lish.shadow_cache );
  if ( i )
...
unsigned char light_get_light_intensity(long idx)
{
  if ( idx )
  {
    if ( lish.lights[idx].flags & LgtF_Allocated )
    {
      return lish.lights[idx].intensity;
    }
    else
    {
    ...
    for (stl_y = start_stl_y; stl_y <= end_stl_y; stl_y++)
    {
      if ( end_stl_x >= start_stl_x )
      {
        ...
        for (stl_x = start_stl_x; stl_x <= end_stl_x; stl_x++)
        {
          ...
          {
            *light_map = lish.global_ambient_light << 8;
          }
          else
          {
            *light_map = 0;
...
void light_stat_refresh() {
    ...
    light_stat_light_map_clear_area(0, 0, kfx_sim_state.map_subtiles_x, kfx_sim_state.map_subtiles_y);
...
void light_set_lights_on(char state)
{
    ...
    {
        // Game rule
        lish.global_ambient_light = kfx_config_state.conf.rules[0].gameplay.global_ambient_light;
        lish.light_enabled = kfx_config_state.conf.rules[0].gameplay.light_enabled;
        lish.light_auto_sync = true;
    } else
    {
        // Fullbright
        lish.global_ambient_light = 32;
        lish.light_enabled = 0;
        lish.light_auto_sync = false;
    }
    ...
    if (get_column_floor_filled_subtiles(col) <= lgt->mappos.z.stl.num)
    {
        ...
        if (&plish->lighting_tables[plish->lighting_tables_count] > &plish->lighting_tables[0])
        {
            do
            {
                ...
                if (lighting_table->delta_x + lgt->mappos.x.stl.num < kfx_sim_state.map_subtiles_x && stl_y < kfx_sim_state.map_subtiles_y)
                {
            ...
            if (!subtile_coords_invalid(stl_x, stl_y))
            {
                ...
                else
                {
                    ...
                    if ( (should_compute_lighting) || (!too_high) )
                    {
                        ...
                        if (floor_filled_stls <= plish->global_ambient_light)
...
static char light_render_light(struct Light* lgt)
{
  ...
  if ( intensity >= lish.global_ambient_light << 8 )
  {
    ...
    lighting_tables_idx = ((intensity - (lish.global_ambient_light << 8)) / intensity_per_tile) + 1;
...
static void light_render_area(MapSubtlCoord startx, MapSubtlCoord starty, MapSubtlCoord endx, MapSubtlCoord endy)
{
  ...
  if ( lish.light_enabled )
  {
    for ( lgt = &lish.lights[kfx_sim_state.thing_lists[TngList_StaticLights].index];
          lgt > lish.lights;
          lgt = &lish.lights[lgt->next_in_list] )
    {
...
{
    if (!lish.light_auto_sync)
        return;

    // Check if any values have changed
    if (
        kfx_config_state.conf.rules[0].gameplay.global_ambient_light != lish.global_ambient_light ||
        kfx_config_state.conf.rules[0].gameplay.light_enabled != lish.light_enabled
    ){

        // GlobalAmbientLight
        if (kfx_config_state.conf.rules[0].gameplay.global_ambient_light != lish.global_ambient_light)
        {
            lish.global_ambient_light = kfx_config_state.conf.rules[0].gameplay.global_ambient_light;
        }

        // LightEnabled
        if (kfx_config_state.conf.rules[0].gameplay.light_enabled != lish.light_enabled)
        {
            lish.light_enabled = kfx_config_state.conf.rules[0].gameplay.light_enabled;
        }
