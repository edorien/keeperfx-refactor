    ...
    if (!can_place_room)
    {
        if (can_build_room_at_slab(player->id_number, player->chosen_room_kind, subtile_slab(stl_x), subtile_slab(stl_y)))
        {
            ...
            if (player->render_roomspace.total_roomspace_cost > dungeon->total_money_owned)
            {
                ...
                {
                    net_callbacks->output_message(SMsg_GoldNotEnough, 0);
                }
            }
        }
        else
                ...
                case CSt_PickAxe:
                    if (!player->render_roomspace.drag_mode)
                    {
                        if (at_limit)
                        {
                            ...
                            {
                                play_non_3d_sample(snd_refusal);
                                net_callbacks->output_message(SMsg_WorkerJobsLimit, 500); // remind the user that the task limit (MAPTASKS_COUNT) has been reached
                            }
                        }
                    }
                    break;
                ...
                if (player->thing_under_hand > 0)
                {
                    if (player->thing_under_hand != player->controlled_thing_idx)
                    {
                        if (is_my_player(player))
                        {
                            ...
                            net_callbacks->turn_on_menu(GMnu_CREATURE_QUERY1);
                        }
