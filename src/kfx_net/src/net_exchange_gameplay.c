...
#include "net_game.h"
#include "packets.h"
// Real usage: game.*/kfx_sim_state.turns_per_second/GKind_*/GOF_*/network_is_active().
// Used to arrive transitively via bflib_datetm.h -> game_legacy.h; made
// explicit after that transitive include was removed (see
...
static int64_t get_current_turn_position_ns(void)
{
    ...
    int64_t nanoseconds_per_turn = (1000000000ULL + kfx_sim_state.turns_per_second / 2) / kfx_sim_state.turns_per_second;
    int64_t position_ns = (int64_t)get_gameturn() * (int64_t)nanoseconds_per_turn;
    if (is_feature_on(Ft_DeltaTime) == true && kfx_net_state.process_turn_time > 0) {
        int64_t turn_phase_ns = (int64_t)((kfx_net_state.process_turn_time - 1) * nanoseconds_per_turn + 0.5);
...
static void update_turn_speed_adjustment(void)
{
    ...
    if (kfx_net_state.frame_skip > 0 || netstate.my_id == SERVER_ID || server_turn_received_at == 0 || kfx_sim_state.turns_per_second <= 0
...
void process_gameplay_chat_message(int player_id, const char *message)
{
    ...
    if (message[0] != '\0') {
        net_callbacks->lua_on_chatmsg(player_id, player->mp_message_text);
        if (player->mp_message_text[0] != cmd_char || !net_callbacks->cmd_exec(player_id, player->mp_message_text + 1) || network_is_active()) {
            sim_feedback->message_add(MsgType_Player, player_id, player->mp_message_text);
...
TbError process_network_turn_sync_message(NetUserId source, const char *buffer, size_t buffer_size)
{
    ...
    int64_t one_way_latency_ns = (int64_t)(((uint64_t)GetPing(source, my_player_number) * 1000000 + 1) / 2);
...
{
    GameTurn expected_turn = get_gameturn() - kfx_net_state.input_lag_turns;
