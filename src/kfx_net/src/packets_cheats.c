        ...
        MapSlabCoord slb_x, MapSlabCoord slb_y)
{
    ...
    switch (player->work_state)
    {
        ...
        snprintf(str, sizeof(str), "%d", player->cheatselection.chosen_experience_level + 1);
        sim_feedback->targeted_message_add(MsgType_Player, player->cheatselection.chosen_player, plyr_idx, 1, str);
        ...
        if (((pckt->control_flags & PCtr_LBtnRelease) != 0) && ((pckt->control_flags & PCtr_MapCoordsValid) != 0))
        {
            if (allowed)
            {
                TbBool effect = (net_callbacks->is_key_pressed(KC_RALT, KMod_DONTCARE));
        ...
        sim_feedback->clear_messages_from_player(MsgType_Blank, -1);
        slb = get_slabmap_block(slb_x, slb_y);
        room = room_get(slb->room_index);
        allowed = (room_exists(room));
        if (allowed)
        {
            sim_feedback->targeted_message_add(MsgType_Blank, 0, plyr_idx, 1, get_string(GUIStr_MnuOk));
        }
        if (((pckt->control_flags & PCtr_LBtnRelease) != 0) && ((pckt->control_flags & PCtr_MapCoordsValid) != 0))
        {
            if (allowed)
            ...
            if (allowed)
            {
                ...
                if (slb->kind >= SlbT_EARTH && slb->kind <= SlbT_CLAIMED)
                {
                    ...
                    switch(slb->kind)
                    {
                        ...
                        case SlbT_EARTH:
                        {
                            if (net_callbacks->is_key_pressed(KC_RSHIFT, KMod_DONTCARE))
                            {
                    ...
                    {
                        effect = (net_callbacks->is_key_pressed(KC_RALT, KMod_DONTCARE));
                    }
                    else
                    {
                        effect = false;
        ...
        if (net_callbacks->process_cheat_heart_health_inputs(&new_health, objst->health))
        {
        ...
        case PSt_PlaceTerrain:
        {
            ...
            {
                player->cheatselection.chosen_player = kfx_config_state.neutral_player_num;
            }
            if (slabst->tooltip_stridx <= GUI_STRINGS_COUNT)
            {
                ...
                sim_feedback->targeted_message_add(MsgType_Player, player->cheatselection.chosen_player, plyr_idx, 1, dis_msg);
            }
            else
            {
                slab_cfgstats = get_slab_kind_stats(player->cheatselection.chosen_terrain_kind);
                sim_feedback->targeted_message_add(MsgType_Player, player->cheatselection.chosen_player, plyr_idx, 1, slab_cfgstats->code_name);
            }
            sim_feedback->clear_messages_from_player(MsgType_Blank, -1);
            if (net_callbacks->is_key_pressed(KC_RSHIFT, KMod_DONTCARE))
            {
                terrain_details ^= 1;
                net_callbacks->clear_key_pressed(KC_RSHIFT);
            }
            if (terrain_details)
            {
                slb = get_slabmap_block(slb_x, slb_y);
            ...
            if (((pckt->control_flags & PCtr_LBtnRelease) != 0) && ((pckt->control_flags & PCtr_MapCoordsValid) != 0))
            {
                ...
                PlayerNumber id = (slab_kind_has_no_ownership(player->cheatselection.chosen_terrain_kind)) ? kfx_config_state.neutral_player_num : player->cheatselection.chosen_player;
    ...
    //      game.???[my_player_number].cheat_mode = 1;
      ...
      case PckA_CheatSwitchExperience:
		...
		case PckA_CheatApplySpell:
		{
			...
			sim_feedback->thing_play_sample(thing, smptbl_idx, NORMAL_PITCH, 0, 3, 0, 4, FULL_LOUDNESS);
