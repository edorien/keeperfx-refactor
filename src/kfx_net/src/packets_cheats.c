    ...
    switch (player->work_state)
    {
        ...
        case PSt_PlaceTerrain:
        {
            ...
            if (slabst->tooltip_stridx <= GUI_STRINGS_COUNT)
            {
                ...
                sim_feedback->targeted_message_add(MsgType_Player, ustate->cheatselection.chosen_player, plyr_idx, 1, dis_msg);
            }
            else
            {
                slab_cfgstats = get_slab_kind_stats(ustate->cheatselection.chosen_terrain_kind);
                sim_feedback->targeted_message_add(MsgType_Player, ustate->cheatselection.chosen_player, plyr_idx, 1, slab_cfgstats->code_name);
            }
            sim_feedback->clear_messages_from_player(MsgType_Blank, -1);
            if (net_callbacks->is_key_pressed(KC_RSHIFT, KMod_DONTCARE))
            {
