...
static int lua_Dead_creatures_return_to_pool(lua_State *L)
{
    ...
    set_flag_value(kfx_sim_state.mode_flags, MFlg_DeadBackToPool, return_to_pool);
    return 0;
}
...
static int lua_Display_timer(lua_State *L)
{
    ...
    kfx_game_state.script_timer_limit = 0;
    kfx_game_state.timer_real = display;
...
static int lua_Bonus_level_time(lua_State *L)
{
    ...
    kfx_game_state.timer_real = clocktime;
    return 0;
}
...
static int lua_Add_bonus_time(lua_State *L)
{
    ...
    kfx_game_state.bonus_time += turns;
    return 0;
}
...
static int lua_Heart_lost_objective(lua_State *L)
{
    ...
    kfx_game_state.heart_lost_message_id = message_id;
    kfx_game_state.heart_lost_message_target = target;
    return 0;
}
static int lua_Heart_lost_quick_objective(lua_State *L)
{
    ...
    snprintf(kfx_sim_state.quick_messages[slot], MESSAGE_TEXT_LEN, "%s", msg);

    kfx_sim_state.heart_lost_display_message = true;
    kfx_game_state.heart_lost_quick_message = true;
    kfx_game_state.heart_lost_message_id = slot;
    kfx_game_state.heart_lost_message_target = target;
    return 0;
}
...
static int lua_Display_countdown(lua_State *L)
{
    ...
    kfx_game_state.script_timer_limit = target;
    kfx_game_state.timer_real = clocktime;
    kfx_game_state.flags_gui |= GGUI_ScriptTimer;
    return 0;
}
...
static int lua_New_creature_type(lua_State* L)
{
    ...
    for (PlayerNumber plyr_idx = 0; plyr_idx < PLAYERS_COUNT; plyr_idx++)
    {
        ...
        if (dungeon_invalid(dungeon))
            continue;
        dungeon->creature_max_level[kfx_config_state.conf.crtr_conf.model_count-1] = CREATURE_MAX_LEVEL + 1;
    }
    return 0;
}
...
static int lua_Set_creature_max_level(lua_State *L)
{
    ...
    for (PlayerNumber i = player_range.start_idx; i < player_range.end_idx; i++)
    {
        ...
        dungeon->creature_max_level[crtr_model%kfx_config_state.conf.crtr_conf.model_count] = max_level;
    }
    return 0;
}
...
static int lua_Change_slab_texture(lua_State *L)
{
    ...
    else
    {
        ...
        kfx_config_state.slab_ext_data[slb_num] = texture_id;
        kfx_config_state.slab_ext_data_initial[slb_num] = texture_id;
    }
