...
FuncIdx get_function_idx(const char *func_name, const struct NamedCommand *Cfuncs) {
    ...
    if (lua_isfunction(Lvl_script, -1)) {
        for (size_t i = 1; i < LUA_FUNCS_MAX; i++) {
            if (strcasecmp(kfx_config_state.conf.lua.lua_funcs[i], func_name) == 0) {
                return -i;
            }

            if (kfx_config_state.conf.lua.lua_funcs[i][0] == '\0') {
                strncpy(kfx_config_state.conf.lua.lua_funcs[i], func_name, LUA_FUNCNAME_LENGTH - 1);
                kfx_config_state.conf.lua.lua_funcs[i][LUA_FUNCNAME_LENGTH - 1] = '\0'; // Ensure null-termination
                return -i;
            }
        }
