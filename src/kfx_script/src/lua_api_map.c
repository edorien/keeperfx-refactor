...
#include "globals.h"
#include "game_legacy.h"         // kfx_sim_state.map_tiles_x/y, kfx_sim_state.pool, kfx_config_state.texture_id
...
static int map_get_field(lua_State *L){
    ...
    if (strcmp(key, "creature_pool") == 0)           
        ...
        lua_pushinteger(L, kfx_sim_state.map_tiles_x);
    } else if (strcmp(key, "height") == 0) {          
        lua_pushinteger(L, kfx_sim_state.map_tiles_y);
    } else if (strcmp(key, "default_texture") == 0) {
        const char *name = get_conf_parameter_text(texture_pack_desc, kfx_config_state.texture_id);
        if (name[0] != '\0') {
            lua_pushstring(L, name);                    // "STANDARD", "ANCIENT", ...
        } else {
            lua_pushinteger(L, kfx_config_state.texture_id - 1);    // convert 1-based storage back to 0-based
        }
    } else {
