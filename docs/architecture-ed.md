...
```
            ...
            │                player/dungeon + ariadne pathfinding        │
            └────────────────────────────────────────────────────────────┘
            ┌────────────────────────────────────────────────────────────┐
...
`#include`. **Depends on:** `kfx_platform`. **35 sources / 38 headers.**
...
- **Callback-struct homes** (see §5): `config.h` (`ConfigReloadCallbacks`),
  ...
  `script_hooks.h`, `sprite_lookup.h`, `dungeon_availability.h`. Each has a
  matching `set_*_callbacks()` and a no-op default implementation in its `.c`.
...
`kfx_config`, `kfx_platform`. **93 sources / 90 headers** — the largest library.
...
- **Pathfinding — Ariadne** (bundled in sim, see §12.1): `ariadne`,
  `ariadne_edge`, `ariadne_findcache`, `ariadne_naviheap`, `ariadne_navitree`,
  `ariadne_points`, `ariadne_regions`, `ariadne_tringls`, `ariadne_update`,
  `ariadne_wallhug`.
...
deliberate (see §1.4 and the `kfx_apploop` note in §1).
...
### 12.1 Ariadne pathfinding — bundled, extraction scoped but not done

Ariadne (`ariadne*`, 10 files) is **bundled inside `kfx_sim`** rather than a
standalone library. The coupling is real but narrow — concentrated in ~4 kinds
of query plus one circular-ownership issue (`struct Navigation` /
`struct Ariadne` embedded inside `CreatureControl`). A concrete query-interface
for standalone extraction is scoped in
[`docs/refactor/stage-06a-ariadne-pathfinding-interface.md`](../refactor/stage-06a-ariadne-pathfinding-interface.md);
standalone extraction remains a stretch goal, not a committed change.
