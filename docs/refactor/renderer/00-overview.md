...
| 3 | [gpu-v2/03-gpu-renderer.md](gpu-v2/03-gpu-renderer.md) — **Phase A landed with stage 2; Phase B re-scoped around stage 4, not started; Phase C now scoped in detail, not started** | Move drawing onto the GPU in phases — Phase A (direct texture upload) shipped with stage 2. Stage 4 then GPU-composited the whole frontend via ImGui/SDLRenderer3, so Phase B is now: frontend backdrop cost, a capture path that includes the overlay, and the in-game HUD (which mostly waits on the in-game-GUI-as-ImGui project). Phase C (3D polygon rasterizer on the GPU) was a stretch goal, deliberately left unscoped — it's now been scoped in detail, informed by a companion review of an external attempt at the same problem ([`docs/merge-checks/opengl-renderer-review.md`](../../merge-checks/opengl-renderer-review.md)). The original `03-gpu-renderer.md` is superseded by `gpu-v2/03-gpu-renderer.md`, which is now the single source for this stage. | 2 |
